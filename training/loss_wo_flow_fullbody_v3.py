                ...
                if self.mask_weight > 0:
                    ...
                    if change_region == 'lower':
                        change_loss_mask = torch.mean(self.upper_ce_parsing(pred_parsing[reconstruct_bs:,...], \
                                            gt_parsing.long()[reconstruct_bs:,0,...])) * self.mask_weight * 0.5
                    elif change_region == 'upper':
                        upper_mask = (parsing_index==1).float()
                        gt_parsing_masked = gt_parsing * (1-upper_mask)
                        change_loss_mask = torch.mean(self.lower_ce_parsing(pred_parsing[reconstruct_bs:,...], \
                                            gt_parsing_masked.long()[reconstruct_bs:,0,...])) * self.mask_weight * 0.5

                        #### debug ######
                        real_img_vis = real_img[reconstruct_bs,...].cpu().numpy().transpose(1,2,0)[...,::-1]
                        real_img_vis = ((real_img_vis+1.0)*127.5).astype(np.uint8)
                        cv2.imwrite('z_debug_denorm_person.png', real_img_vis)

                        real_clothes_img_vis = real_clothes_img[reconstruct_bs,...].cpu().numpy().transpose(1,2,0)[...,::-1]
                        real_clothes_img_vis = ((real_clothes_img_vis+1.0)*127.5).astype(np.uint8)
                        cv2.imwrite('z_debug_denorm_clothes.png', real_clothes_img_vis)

                        denorm_upper_vis = denorm_upper_input[reconstruct_bs,...].cpu().numpy().transpose(1,2,0)[...,::-1]
                        denorm_upper_vis = ((denorm_upper_vis+1.0)*127.5).astype(np.uint8)
                        cv2.imwrite('z_debug_denorm_upper.png', denorm_upper_vis)
                        denorm_lower_vis = denorm_lower_input[reconstruct_bs,...].cpu().numpy().transpose(1,2,0)[...,::-1]
                        denorm_lower_vis = ((denorm_lower_vis+1.0)*127.5).astype(np.uint8)
                        cv2.imwrite('z_debug_denorm_lower.png', denorm_lower_vis)

                        gt_parsing_vis = gt_parsing[reconstruct_bs,...].cpu().numpy().transpose(1,2,0)
                        gt_parsing_vis = (gt_parsing_vis/5*255).astype(np.uint8)
                        cv2.imwrite('z_debug_gt_parsing.png', gt_parsing_vis)

                        pred_parsing_vis = parsing_index[reconstruct_bs,...].cpu().numpy().transpose(1,2,0)
                        pred_parsing_vis = (pred_parsing_vis/5*255).astype(np.uint8)
                        cv2.imwrite('z_debug_pred_parsing.png', pred_parsing_vis)
                        ####################
                    ...
                    #     for ll in range(15):
                    #         norm_patch = style_input_vis[...,ll*3:(ll+1)*3]
