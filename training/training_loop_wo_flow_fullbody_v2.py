...
def setup_snapshot_image_grid(training_set, device, random_seed=0):
    ...
    # Load data.
    images, poses, _, _, norm_img, norm_img_lower, _, _, M_invs, _, _, _, norm_clothes_mask, norm_clothes_mask_lower, retain_masks = zip(*[training_set[i] for i in grid_indices])
...
def denorm_training_img(style_inputs, M_invs, norm_masks, norm_mask_lowers):
    denorm_upper_input = []
    denorm_lower_input = []
    denorm_upper_mask = []
    denorm_lower_mask = []

    bs = style_inputs.size(0)
    for ii in range(bs):
        style_input = style_inputs[ii].cpu().numpy().transpose(1,2,0)
        style_input = ((style_input + 1.0) * 127.5).astype(np.uint8)

        # for jj in range(15):
        #     style_np = style_input[...,jj*3:(jj+1)*3][...,[2,1,0]]
        #     cv2.imwrite('z_debug_norm_%d.png' % jj, style_np)

        style_input_upper = style_input[...,0:10*3]
        style_input_lower = style_input[...,10*3:]
        norm_mask_upper = norm_masks[ii].cpu().numpy().transpose(1,2,0)
        norm_mask_lower = norm_mask_lowers[ii].cpu().numpy().transpose(1,2,0)
        M_inv = M_invs[ii].cpu().numpy()

        denorm_upper_img = np.zeros((512,512,3),dtype=np.uint8)
        denorm_lower_img = np.zeros((512,512,3),dtype=np.uint8)
        kernel = np.ones((5,5),np.uint8)

        for jj in range(M_inv.shape[0]):
            M = M_inv[jj]
            if M.sum() == 0:
                continue
            norm_patch = style_input_upper[...,jj*3:(jj+1)*3]
            norm_mask_patch = norm_mask_upper[...,jj*3:(jj+1)*3]

            denorm_patch = cv2.warpPerspective(norm_patch,M,(512,512),borderMode=cv2.BORDER_CONSTANT)
            denorm_clothes_mask_patch = cv2.warpPerspective(norm_mask_patch,M,(512,512),borderMode=cv2.BORDER_CONSTANT)
            denorm_clothes_mask_patch = cv2.erode(denorm_clothes_mask_patch, kernel, iterations=1)[...,0:1]
            denorm_clothes_mask_patch = (denorm_clothes_mask_patch==255).astype(np.uint8)
            denorm_upper_img = denorm_patch * denorm_clothes_mask_patch + denorm_upper_img * (1-denorm_clothes_mask_patch)

            # norm_patch_np = norm_patch[...,[2,1,0]]
            # denorm_patch_np = denorm_patch[...,[2,1,0]]
            # norm_mask_patch_np = norm_mask_patch[...,[2,1,0]]
            # cv2.imwrite('z_debug_norm_mask_patch_%d.png' % jj, norm_mask_patch_np)
            # cv2.imwrite('z_debug_norm_patch_%d.png' % jj, norm_patch_np)
            # cv2.imwrite('z_debug_denorm_patch_%d.png' % jj, denorm_patch_np)

            if jj == 0 or jj >= 6:
                if jj == 0:
                    norm_patch_lower = style_input_lower[...,jj*3:(jj+1)*3]
                    norm_mask_patch_lower = norm_mask_lower[...,jj*3:(jj+1)*3]
                else:
                    norm_patch_lower = style_input_lower[...,(jj-6+1)*3:(jj-6+2)*3]
                    norm_mask_patch_lower = norm_mask_lower[...,(jj-6+1)*3:(jj-6+2)*3]

                denorm_patch_lower = cv2.warpPerspective(norm_patch_lower,M,(512,512),borderMode=cv2.BORDER_CONSTANT)
                denorm_clothes_mask_patch_lower = cv2.warpPerspective(norm_mask_patch_lower,M,(512,512),borderMode=cv2.BORDER_CONSTANT)
                denorm_clothes_mask_patch_lower = cv2.erode(denorm_clothes_mask_patch_lower, kernel, iterations=1)[...,0:1]
                denorm_clothes_mask_patch_lower = (denorm_clothes_mask_patch_lower==255).astype(np.uint8)
                denorm_lower_img = denorm_patch_lower * denorm_clothes_mask_patch_lower + denorm_lower_img * (1-denorm_clothes_mask_patch_lower)
        ...
        denorm_upper_input.append(denorm_upper_img)
        denorm_lower_input.append(denorm_lower_img)
        denorm_upper_mask.append(denorm_upper_clothes_mask)
        denorm_lower_mask.append(denorm_lower_clothes_mask)

    denorm_upper_input = np.concatenate(denorm_upper_input, axis=0)
    denorm_lower_input = np.concatenate(denorm_lower_input, axis=0)
    denorm_upper_mask = np.concatenate(denorm_upper_mask, axis=0)
    denorm_lower_mask = np.concatenate(denorm_lower_mask, axis=0)

    denorm_upper_input = torch.Tensor(denorm_upper_input).to(torch.float32) / 127.5 - 1.0
    denorm_lower_input = torch.Tensor(denorm_lower_input).to(torch.float32) / 127.5 - 1.0
    denorm_upper_mask = torch.Tensor(denorm_upper_mask).to(torch.float32)
    denorm_lower_mask = torch.Tensor(denorm_lower_mask).to(torch.float32)

    return denorm_upper_input, denorm_lower_input, denorm_upper_mask, denorm_lower_mask
...
def training_loop(
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            # 读取模型输入数据和GT
            phase_real, phase_pose, phase_norm_img, phase_norm_img_lower, _, _, phase_denorm_upper_img, phase_denorm_lower_img, phase_M_invs, \
                phase_gt_parsing, phase_denorm_upper_mask, phase_denorm_lower_mask, phase_norm_mask, phase_norm_mask_lower, \
                phase_retain_mask = next(training_set_iterator)
            ...
            # phase_M_invs = phase_M_invs.to(torch.float32)
            # phase_norm_mask = phase_norm_mask.to(torch.float32)
            # phase_norm_mask_lower = phase_norm_mask_lower.to(torch.float32)
            phase_M_invs_tensor = phase_M_invs.split(batch_gpu)
            phase_norm_mask_tensor = phase_norm_mask.split(batch_gpu)
            phase_norm_mask_lower_tensor = phase_norm_mask_lower.split(batch_gpu)
        ...
        for phase, phase_gen_z in zip(phases, all_gen_z):
            ...
            for round_idx, (real_img, gen_z, style_input, retain, pose, denorm_upper_input, denorm_lower_input, \
                            denorm_upper_mask, denorm_lower_mask, gt_parsing, M_invs, norm_masks, norm_mask_lowers) \
                    in enumerate(zip(phase_real_tensor, phase_gen_z, phase_parts_tensor, \
                                     ...
                                     phase_M_invs_tensor, phase_norm_mask_tensor, phase_norm_mask_lower_tensor)):
                ...
                reconstruct_style_input = style_input[0:reconstruct_bs,...]
                reconstruct_denorm_upper_input = denorm_upper_input[0:reconstruct_bs,...]
                reconstruct_denorm_lower_input = denorm_lower_input[0:reconstruct_bs,...]
                reconstruct_denorm_upper_mask = denorm_upper_mask[0:reconstruct_bs,...]
                reconstruct_denorm_lower_mask = denorm_lower_mask[0:reconstruct_bs,...]
                try_on_style_input = style_input[reconstruct_bs:,...]

                if random.random() > 0.5:
                    retain_region = 'upper'
                    target_lower_norm_patches = reconstruct_style_input[0:try_on_bs,10*3:,...].clone()
                    try_on_style_input[:,10*3:,...] = target_lower_norm_patches
                    norm_mask = norm_masks[reconstruct_bs:,...]
                    norm_mask_lower = norm_mask_lowers[0:try_on_bs,...]
                else:
                    retain_region = 'lower'
                    target_upper_norm_patches = reconstruct_style_input[0:try_on_bs,0:10*3,...].clone()
                    try_on_style_input[:,0:10*3,...] = target_upper_norm_patches
                    norm_mask = norm_masks[0:try_on_bs,...]
                    norm_mask_lower = norm_mask_lowers[reconstruct_bs:,...]

                M_inv = M_invs[reconstruct_bs:,...]

                try_on_denorm_upper_input, try_on_denorm_lower_input, try_on_denorm_upper_mask, \
                    try_on_denorm_lower_mask = denorm_training_img(try_on_style_input, M_inv, norm_mask, norm_mask_lower)

                try_on_denorm_upper_input = try_on_denorm_upper_input.to(real_img.device)
                try_on_denorm_lower_input = try_on_denorm_lower_input.to(real_img.device)
                try_on_denorm_upper_mask = try_on_denorm_upper_mask.to(real_img.device)
                try_on_denorm_lower_mask = try_on_denorm_lower_mask.to(real_img.device)

                style_input = torch.cat([reconstruct_style_input, try_on_style_input], dim=0)
                denorm_upper_input = torch.cat([reconstruct_denorm_upper_input, try_on_denorm_upper_input],dim=0)
                denorm_lower_input = torch.cat([reconstruct_denorm_lower_input, try_on_denorm_lower_input],dim=0)
                denorm_upper_mask = torch.cat([reconstruct_denorm_upper_mask, try_on_denorm_upper_mask],dim=0)
                denorm_lower_mask = torch.cat([reconstruct_denorm_lower_mask, try_on_denorm_lower_mask],dim=0)
                
                ####### debug ########
                # for ii in range(gpu_bs):
                #     image_np = real_img[ii].cpu().numpy().transpose(1,2,0)[...,[2,1,0]]
                #     image_np = ((image_np + 1.0) * 127.5).astype(np.uint8)
                #     pose_np = pose[ii].cpu().numpy().transpose(1,2,0)[...,[2,1,0]]
                #     pose_np = ((pose_np + 1.0) * 127.5).astype(np.uint8)
                #     pose_mask = (np.sum(pose_np,axis=2,keepdims=True)>0).astype(np.uint8)
                #     denorm_upper_np = denorm_upper_input[ii].cpu().numpy().transpose(1,2,0)[...,[2,1,0]]
                #     denorm_upper_np = ((denorm_upper_np+1.0)*127.5).astype(np.uint8)
                #     denorm_lower_np = denorm_lower_input[ii].cpu().numpy().transpose(1,2,0)[...,[2,1,0]]
                #     denorm_lower_np = ((denorm_lower_np+1.0)*127.5).astype(np.uint8)

                #     denorm_upper_np = denorm_upper_np * (1-pose_mask) + pose_np * pose_mask
                #     denorm_lower_np = denorm_lower_np * (1-pose_mask) + pose_np * pose_mask

                #     cv2.imwrite('z_debug_image_%d.png' % ii, image_np)
                #     cv2.imwrite('z_debug_denonrm_upper_%d.png' % ii, denorm_upper_np)
                #     cv2.imwrite('z_debug_denonrm_lower_%d.png' % ii, denorm_lower_np)
                ######################
                ...
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, gen_z=gen_z, style_input=style_input, 
                                          ...
                                          denorm_lower_mask=denorm_lower_mask, gt_parsing=gt_parsing, 
                                          retain_region=retain_region, sync=sync, gain=gain)
