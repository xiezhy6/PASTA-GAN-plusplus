...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, G_const_encoding, G_style_encoding, 
        ...
        upper_class_weight = torch.FloatTensor([0,1,0,1,0,0]).to(device)
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                if self.mask_weight > 0:
                    ...
                    parsing_index = torch.argmax(softmax(pred_parsing.detach()), dim=1)[:,None,...].to(gt_parsing.dtype)

                    if change_region == 'lower':
                        # gt_upper_mask = (gt_parsing == 1).to(gt_parsing.dtype) + (gt_parsing == 3).to(gt_parsing.dtype)

                        # fake_gt_parsing = parsing_index * (1-gt_upper_mask)
                        # fake_gt_parsing = (gt_parsing == 1).to(gt_parsing.dtype) + \
                        #                   (gt_parsing == 3).to(gt_parsing.dtype) + \
                        #                   (fake_gt_parsing == 2).to(gt_parsing.dtype) + \
                        #                   (fake_gt_parsing == 4).to(gt_parsing.dtype) + \
                        #                   (fake_gt_parsing == 5).to(gt_parsing.dtype)
                        fake_gt_parsing = (gt_parsing == 1).to(gt_parsing.dtype) * 1 + \
                                          (gt_parsing == 3).to(gt_parsing.dtype) * 3

                        change_loss_mask = torch.mean(self.upper_ce_parsing(pred_parsing[reconstruct_bs:,...], \
                                            fake_gt_parsing.long()[reconstruct_bs:,0,...])) * self.mask_weight
                        ...
                        partial_gt_parsing = gt_parsing * (1-fake_upper_mask)

                        # fake_gt_parsing = (parsing_index==1).to(gt_parsing.dtype) + \
                        #                   (parsing_index==3).to(gt_parsing.dtype) + \
                        #                   (parsing_index==5).to(gt_parsing.dtype) + \
                        #                   (partial_gt_parsing==2).to(gt_parsing.dtype) + \
                        #                   (partial_gt_parsing==4).to(gt_parsing.dtype) 

                        fake_gt_parsing = (partial_gt_parsing==2).to(gt_parsing.dtype) * 2 + \
