...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, G_const_encoding, G_style_encoding, 
                 D, D_parsing, augment_pipe=None, style_mixing_prob=0.9, r1_gamma=10, 
        ...
        class_weight  = torch.FloatTensor([1,2,2,3,3,3,3,3]).to(device)
    ...
    def run_G(self, z, c, pose, const_feats, denorm_upper_mask, denorm_lower_mask, denorm_upper_input, denorm_lower_input, retain, face_mask, sync):
        cat_feats = {}
        ...
        with misc.ddp_sync(self.G_synthesis, sync):
            img, finetune_img, pred_parsing = self.G_synthesis(ws, pose_feat, cat_feats, denorm_upper_input, denorm_lower_input, \
                                                                denorm_upper_mask, denorm_lower_mask, retain, face_mask)
    ...
    def run_D(self, img, pose, c, sync):
        ...
        with misc.ddp_sync(self.D, sync):
            logits = self.D(torch.cat([img, pose],dim=1), c)
        return logits
    ...
    def run_D_parsing(self, parsing, pose, c, sync):
        ...
        with misc.ddp_sync(self.D_parsing, sync):
            logits = self.D_parsing(torch.cat([parsing,pose],dim=1), c)
        return logits
    ...
    def accumulate_gradients(self, phase, real_img, gen_z, style_input, retain, pose, denorm_upper_input, denorm_lower_input, denorm_upper_mask, denorm_lower_mask, \
                             gt_parsing, face_mask, sync, gain):
        assert phase in ['Gmain', 'Greg', 'Gboth', 'Dmain', 'Dreg', 'Dboth', 'D_parsingmain', 'D_parsingreg', 'D_parsingboth']
        do_Gmain = (phase in ['Gmain', 'Gboth'])
        do_Dmain = (phase in ['Dmain', 'Dboth'])
        do_Gpl   = (phase in ['Greg', 'Gboth']) and (self.pl_weight != 0)
        do_Dr1   = (phase in ['Dreg', 'Dboth']) and (self.r1_gamma != 0)
        do_DPmain = (phase in ['D_parsingmain', 'D_parsingboth'])
        do_DPr1 = (phase in ['D_parsingreg', 'D_parsingboth']) and (self.r1_gamma != 0)
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, gen_finetune_img, pred_parsing, _gen_ws = self.run_G(gen_z, gen_c, pose, cat_feats, denorm_upper_mask, denorm_lower_mask, \
                                                                            denorm_upper_input, denorm_lower_input, retain, face_mask, sync=(sync and not do_Gpl)) # May get synced by Gpl.
                # pred_parsing_onehot = self.softmax(pred_parsing)
                pred_parsing_onehot = F.gumbel_softmax(pred_parsing, tau=1, hard=True, dim=1)
                ...
                gen_finetune_logits = self.run_D(gen_finetune_img, pose, gen_c, sync=False)
                parsing_logits = self.run_D_parsing(pred_parsing_onehot, pose, gen_c,sync=False)
                ...
                training_stats.report('Loss/scores/fake_parsing', parsing_logits)
                training_stats.report('Loss/signs/fake_parsing', parsing_logits.sign())
                loss_Gmain_parsing = torch.nn.functional.softplus(-parsing_logits) # -log(sigmoid(gen_logits))
                loss_Gmain_parsing = loss_Gmain_parsing.mean()
                training_stats.report('Loss/G/loss_parsing', loss_Gmain_parsing)
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, gen_finetune_img, _, _gen_ws = self.run_G(gen_z, gen_c, pose, cat_feats, denorm_upper_mask, denorm_lower_mask, \
                                                                    denorm_upper_input, denorm_lower_input, retain, face_mask, sync=False)
                gen_logits = self.run_D(gen_img, pose, gen_c, sync=False) # Gets synced by loss_Dreal.
                gen_finetune_logits = self.run_D(gen_finetune_img, pose, gen_c, sync=False)
        ...
        if do_DPmain:
            with torch.autograd.profiler.record_function('Dparsing_forward'):
                _, _, pred_parsing, _ = self.run_G(gen_z, gen_c, pose, cat_feats, denorm_upper_mask, denorm_lower_mask, \
                                                    denorm_upper_input, denorm_lower_input, retain, face_mask, sync=False)
                ...
                training_stats.report('Loss/scores/fake_parsing', parsing_logits)
                training_stats.report('Loss/signs/fake_parsing', parsing_logits.sign())
                loss_Dparsing = torch.nn.functional.softplus(parsing_logits) # -log(1 - sigmoid(gen_logits))
        ...
        if do_DPmain or do_DPr1:
            name = 'DPreal_DPr1' if do_DPmain and do_DPr1 else 'DPreal' if do_DPmain else 'DPr1'
            with torch.autograd.profiler.record_function(name + '_forward'):
                bg_mask = (gt_parsing==0).to(gt_parsing.dtype).to(gt_parsing.device)
                upper_mask = (gt_parsing==1).to(gt_parsing.dtype).to(gt_parsing.device)
                lower_mask = (gt_parsing==2).to(gt_parsing.dtype).to(gt_parsing.device)
                lh_mask = (gt_parsing==3).to(gt_parsing.dtype).to(gt_parsing.device)
                rh_mask = (gt_parsing==4).to(gt_parsing.dtype).to(gt_parsing.device)
                ll_mask = (gt_parsing==5).to(gt_parsing.dtype).to(gt_parsing.device)
                rl_mask = (gt_parsing==6).to(gt_parsing.dtype).to(gt_parsing.device)
                neck_mask = (gt_parsing==7).to(gt_parsing.dtype).to(gt_parsing.device)

                gt_parsing_tmp = torch.cat([bg_mask,upper_mask,lower_mask,lh_mask,rh_mask,ll_mask,rl_mask,neck_mask],dim=1).detach().requires_grad_(do_DPr1)
                real_parsing_logit = self.run_D_parsing(gt_parsing_tmp, pose, real_c, sync=sync)
                # real_parsing_logit = self.run_D_parsing(gt_parsing, pose, real_c, sync=sync)
                training_stats.report('Loss/scores/real_parsing', real_parsing_logit)
                training_stats.report('Loss/signs/real_parsing', real_parsing_logit.sign())           

                loss_DPreal = 0
                if do_DPmain:
                    loss_DPreal = torch.nn.functional.softplus(-real_parsing_logit) # -log(sigmoid(real_logits))
                    training_stats.report('Loss/D_parsing/loss', loss_Dparsing + loss_DPreal)

                loss_DPr1 = 0
                if do_DPr1:
                    with torch.autograd.profiler.record_function('DP_r1_grads'), conv2d_gradfix.no_weight_gradients():
                        DP_r1_grads = torch.autograd.grad(outputs=[real_parsing_logit.sum()], inputs=[gt_parsing_tmp], create_graph=True, only_inputs=True)[0]
                        # DP_r1_grads = torch.autograd.grad(outputs=[real_parsing_logit.sum()], inputs=[gt_parsing], create_graph=True, only_inputs=True)[0]
            ...
            with torch.autograd.profiler.record_function(name + '_backward'):
                (real_parsing_logit * 0 + loss_DPreal + loss_DPr1).mean().mul(gain).backward()
