...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, G_const_encoding, G_style_encoding, 
                 D, G_ema_fixed, augment_pipe=None, style_mixing_prob=0.9, r1_gamma=10, 
        ...
        upper_class_weight = torch.FloatTensor([1,2,1,2,1,1]).to(device)
        self.upper_ce_parsing = torch.nn.CrossEntropyLoss(ignore_index=255,weight=upper_class_weight)

        lower_class_weight = torch.FloatTensor([1,1,2,1,2,1]).to(device)
        self.lower_ce_parsing = torch.nn.CrossEntropyLoss(ignore_index=255,weight=lower_class_weight)
    ...
    def accumulate_gradients(self, phase, real_img, real_clothes_img, gen_z, style_input, retain, pose, denorm_upper_input, denorm_lower_input, denorm_upper_mask, denorm_lower_mask, \
                             gt_parsing, random_gt_parsing, change_region, sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                bs = real_img.size(0)
                try_on_bs = bs // 3
                reconstruct_bs = bs-try_on_bs

                _, _, pred_gt_parsing = self.G_ema_fixed(z=gen_z[reconstruct_bs:,...], c=style_input[reconstruct_bs:,...], \
                                                        retain=retain[reconstruct_bs:,...], pose=pose[reconstruct_bs:,...], denorm_upper_input=denorm_upper_input[reconstruct_bs:,...], \
                                                        denorm_lower_input=denorm_lower_input[reconstruct_bs:,...], denorm_upper_mask=denorm_upper_mask[reconstruct_bs:,...], \
                                                        denorm_lower_mask=denorm_lower_mask[reconstruct_bs:,...])

                # l1 loss
                loss_G_L1 = 0
                loss_G_finetune_L1 = 0
                if self.l1_weight > 0:
                    loss_G_L1 = torch.nn.L1Loss()(gen_img[0:reconstruct_bs,...], real_img[0:reconstruct_bs,...]) * self.l1_weight
                    loss_G_finetune_L1 = torch.nn.L1Loss()(gen_finetune_img[0:reconstruct_bs,...], real_img[0:reconstruct_bs,...]) * self.l1_weight
                ...
                if self.mask_weight > 0:
                    loss_mask = torch.mean(self.ce_parsing(pred_parsing[0:reconstruct_bs,...], gt_parsing.long()[0:reconstruct_bs,0,...])) * self.mask_weight

                    softmax = torch.nn.Softmax(dim=1)
                    parsing_index = torch.argmax(softmax(pred_gt_parsing.detach()), dim=1)[:,None,...].to(gt_parsing.dtype)

                    if change_region == 'lower':
                        gt_upper_mask = (gt_parsing[reconstruct_bs:,...] == 1).to(gt_parsing.dtype) + \
                                        (gt_parsing[reconstruct_bs:,...] == 3).to(gt_parsing.dtype)

                        fake_gt_parsing = parsing_index * (1-gt_upper_mask)
                        fake_gt_parsing = (gt_parsing[reconstruct_bs:,...] == 1).to(gt_parsing.dtype) * 1 + \
                                          (gt_parsing[reconstruct_bs:,...] == 3).to(gt_parsing.dtype) * 3 + \
                                          (fake_gt_parsing == 2).to(gt_parsing.dtype) * 2 + \
                                          (fake_gt_parsing == 4).to(gt_parsing.dtype) * 4 + \
                                          (fake_gt_parsing == 5).to(gt_parsing.dtype) * 5

                        change_loss_mask = torch.mean(self.upper_ce_parsing(pred_parsing[reconstruct_bs:,...], \
                                            fake_gt_parsing.long()[:,0,...])) * self.mask_weight
                    
                    elif change_region == 'upper':
                        fake_upper_mask = (parsing_index==1).to(gt_parsing.dtype) + \
                                          (parsing_index==3).to(gt_parsing.dtype) + \
                                          (parsing_index==5).to(gt_parsing.dtype)
                        partial_gt_parsing = gt_parsing[reconstruct_bs:,...] * (1-fake_upper_mask)

                        fake_gt_parsing = (parsing_index==1).to(gt_parsing.dtype) * 1 + \
                                          (parsing_index==3).to(gt_parsing.dtype) * 3 + \
                                          (parsing_index==5).to(gt_parsing.dtype) * 5 + \
                                          (partial_gt_parsing==2).to(gt_parsing.dtype) * 2 + \
                                          (partial_gt_parsing==4).to(gt_parsing.dtype) * 4

                        change_loss_mask = torch.mean(self.lower_ce_parsing(pred_parsing[reconstruct_bs:,...], \
                                            fake_gt_parsing.long()[:,0,...])) * self.mask_weight
                    ...
                    #     if kk != (bs -1):
                    #         continue
                    #     real_img_vis = real_img[kk].cpu().numpy().transpose(1,2,0)[...,::-1]
                    #     real_img_vis = ((real_img_vis+1.0) * 127.5).astype(np.uint8)
                    #     cv2.imwrite('z_debug_person_%d.png' % kk, real_img_vis)
                    #     real_clothes_img_vis = real_clothes_img[kk].cpu().numpy().transpose(1,2,0)[...,::-1]
                    #     real_clothes_img_vis = ((real_clothes_img_vis+1.0) * 127.5).astype(np.uint8)
                    #     cv2.imwrite('z_debug_clothes_%d.png' % kk, real_clothes_img_vis)
                    #     pose_vis = pose[kk][0:3,...].cpu().numpy().transpose(1,2,0)[...,::-1]
                    #     pose_vis = ((pose_vis+1.0) * 127.5).astype(np.uint8)
                    #     pose_mask = (np.sum(pose_vis, axis=2, keepdims=True)>0).astype(np.uint8)

                    #     denorm_upper_vis = denorm_upper_input[kk].cpu().numpy().transpose(1,2,0)[...,::-1]
                    ...
                    #     cv2.imwrite('z_debug_denorm_upper_%d.png' % kk, denorm_upper_vis)
                    #     denorm_lower_vis = denorm_lower_input[kk].cpu().numpy().transpose(1,2,0)[...,::-1]
                    ...
                    #     cv2.imwrite('z_debug_denorm_lower_%d.png' % kk, denorm_lower_vis)

                    #     denorm_upper_mask_vis = denorm_upper_mask[kk].cpu().numpy().transpose(1,2,0)
                    #     denorm_upper_mask_vis = np.concatenate([denorm_upper_mask_vis,denorm_upper_mask_vis,denorm_upper_mask_vis], axis=2)
                    #     denorm_upper_mask_vis = (denorm_upper_mask_vis * 255).astype(np.uint8)
                    #     cv2.imwrite('z_debug_denorm_upper_mask_%d.png' % kk, denorm_upper_mask_vis)

                    #     denorm_lower_mask_vis = denorm_lower_mask[kk].cpu().numpy().transpose(1,2,0)
                    #     denorm_lower_mask_vis = np.concatenate([denorm_lower_mask_vis,denorm_lower_mask_vis,denorm_lower_mask_vis], axis=2)
                    #     denorm_lower_mask_vis = (denorm_lower_mask_vis * 255).astype(np.uint8)
                    #     cv2.imwrite('z_debug_denorm_lower_mask_%d.png' % kk, denorm_lower_mask_vis)


                    #     predict_parsing_vis = parsing_index[kk].cpu().numpy().transpose(1,2,0)
                    #     predict_parsing_vis = (predict_parsing_vis / 5 * 255).astype(np.uint8)
                    #     predict_parsing_vis = np.concatenate([predict_parsing_vis,predict_parsing_vis,predict_parsing_vis],axis=2)
                    #     cv2.imwrite('z_debug_predict_parsing_%d.png' % kk, predict_parsing_vis)

                    #     if change_region == 'lower':
                    #         gt_parsing_vis = gt_parsing[kk].cpu().numpy().transpose(1,2,0)
                    #     else:
                    #         gt_parsing_vis = gt_parsing_masked[kk].cpu().numpy().transpose(1,2,0)
                    #     gt_parsing_vis = (gt_parsing_vis / 5 * 255).astype(np.uint8)
                    #     gt_parsing_vis = np.concatenate([gt_parsing_vis,gt_parsing_vis,gt_parsing_vis],axis=2)
                    #     cv2.imwrite('z_debug_gt_parsing_%d.png' % (kk), gt_parsing_vis)

                    #     style_input_vis = style_input[kk].cpu().numpy().transpose(1,2,0)
                    #     style_input_vis = ((style_input_vis+1.0) * 127.5).astype(np.uint8)
                    #     for ll in range(15):
                    #         norm_patch = style_input_vis[...,ll*3:(ll+1)*3][...,::-1]
                    #         cv2.imwrite('z_debug_norm_patch_%d.png' % ll, norm_patch)

                    # #### visualization for debug ########

                training_stats.report('Loss/G/mask_loss', loss_mask)
                training_stats.report('Loss/G/change_mask_loss', change_loss_mask)

                # vgg loss
                loss_G_VGG = 0
                loss_G_finetune_VGG = 0
                if self.vgg_weight > 0:
                    loss_G_VGG = self.criterionVGG(gen_img[0:reconstruct_bs,...], real_img[0:reconstruct_bs,...]) * self.vgg_weight
                    loss_G_VGG = loss_G_VGG.mean()

                    loss_G_finetune_VGG = self.criterionVGG(gen_finetune_img[0:reconstruct_bs,...], real_img[0:reconstruct_bs,...]) * self.vgg_weight
