...
def setup_snapshot_image_grid(training_set, device, random_seed=0):
    ...
    # from util_functions import vis_numpy
    # for ii in range(gnum):
    #     image = images[ii]
    #     norm_im = norm_img[ii]

    #     vis_numpy(image.transpose(1,2,0), name='z_debug_image.jpg')
    #     for jj in range(10):
    #         patch = norm_im[jj*3:(jj+1)*3,...]
    #         vis_numpy(patch.transpose(1,2,0), name='z_debug_patch_%d.jpg' % jj)
...
def training_loop(
    ...
    if rank == 0:
        ...
        z = torch.empty([batch_gpu, G.z_dim], device=device)
        c = torch.empty([batch_gpu, 30 * 2, 128, 128], device=device)
        c_feature = torch.empty([batch_gpu, 512], device=device)
