...
def setup_snapshot_image_grid(training_set, device, random_seed=0):
    ...
    # Load data.
    images, poses, norm_img, norm_img_lower,_, _, _, M_invs, _, _, _, norm_clothes_mask, \
            norm_clothes_mask_lower, retain_masks, skins, lower_label_map = zip(*[training_set[i] for i in grid_indices])
    ...
    grid_denorm_upper_ims, grid_denorm_lower_ims, grid_denorm_upper_masks, grid_denorm_lower_masks = zip(*[denorm_clothes(norm_patches, norm_patches_lower, M_invs, \
                                                                                                        norm_clothes_mask, norm_clothes_mask_lower, \
    ...
    norm_img = np.concatenate([norm_img, norm_img_lower, lower_label_map], axis=1)
...
def training_loop(
    ...
    common_kwargs_D_parsing = dict(c_dim=512, img_resolution=training_set.resolution, img_channels=(6+6))
    ...
    if rank == 0:
        ...
        c = torch.empty([batch_gpu, (10*3+5*3+1), 128, 128], device=device)
        ...
        misc.print_module_summary(D_parsing, [torch.cat([img[1], pose],dim=1), c_feature])
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            # 读取模型输入数据和GT
            phase_real, phase_pose, phase_norm_img, _,phase_norm_img_lower, phase_denorm_upper_img, phase_denorm_lower_img, _, \
                phase_gt_parsing, phase_denorm_upper_mask, phase_denorm_lower_mask, _, _, phase_retain_mask, \
                phase_skin, phase_lower_label_map = next(training_set_iterator)
            ...
            phase_parts_tensor = torch.cat([phase_parts_tensor,phase_parts_lower_tensor,phase_lower_label_map_tensor],dim=1)
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            ...
            for z, c, retain, denorm_upper_im, denorm_lower_im, denorm_upper_ma, denorm_lower_ma, pose in zip(grid_z, \
                ...
                finetune_fake_image, pred_parsing = G_ema(z=z, c=c, retain=retain, pose=pose, denorm_upper_input=denorm_upper_im, denorm_lower_input=denorm_lower_im, \
