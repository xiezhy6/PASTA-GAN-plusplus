...
def denorm_clothes(norm_patches, norm_patches_lower, norm_clothes_mask, norm_clothes_mask_lower, \
                lower_label_maps, gt_parsings, lower_conditions_for_train, lower_clothes_upper_bounds_for_test, \
                Ms, M_invs, col, row, gnum):
    ...
    upper_lower_norm_patches = np.concatenate([upper_norm_patches, lower_norm_patches, lower_label_map],axis=2)
    ...
    if row < gap:
        ...
        lower_condition_for_train = lower_conditions_for_train[col].transpose(1,2,0)
        lower_clothes_upper_bound = np.zeros((512,512,1))
        lower_clothes_upper_bound_tmp = np.zeros((512,512,1))
        part_index_list = [0,6,8]
        for ii, part_index in enumerate(part_index_list):
            M_inv = M_invs[row,part_index]
            if np.sum(M_inv) == 0:
                continue
            lower_condition_patch = lower_condition_for_train[...,ii:(ii+1)]
            denorm_lower_condition = cv2.warpPerspective(lower_condition_patch,M_inv,(512,512),borderMode=cv2.BORDER_CONSTANT)[...,np.newaxis]
            lower_clothes_upper_bound_tmp += denorm_lower_condition
        lower_clothes_upper_bound_mask = (lower_clothes_upper_bound_tmp>0).astype(np.uint8)
        bbox = mask_to_bbox(lower_clothes_upper_bound_mask.copy())
        if bbox is not None:
            upper_bound = bbox[1]
            lower_clothes_upper_bound[upper_bound:, ...] += 255
    else:
        lower_clothes_upper_bound = lower_clothes_upper_bounds_for_test[row].transpose(1,2,0)
        denorm_upper_img_tmp = denorm_upper_img[0].transpose(1,2,0)
        denorm_upper_img_tmp_mask = (np.sum(denorm_upper_img_tmp, axis=2, keepdims=True)>0).astype(np.uint8)
        bbox = mask_to_bbox(denorm_upper_img_tmp_mask.copy())
        if bbox is not None:
            lower_bound = bbox[3]
            lower_clothes_upper_bound[0:lower_bound,...] *= 0

    lower_condition_for_test = []
    part_index_list = [0,6,8]
    for ii, part_index in enumerate(part_index_list):
        M = Ms[row,part_index]
        part_lower_condition_for_test = cv2.warpPerspective(lower_clothes_upper_bound, M, (128,128), borderMode = cv2.BORDER_CONSTANT)[...,np.newaxis]
        lower_condition_for_test.append(part_lower_condition_for_test)
    lower_condition_for_test = np.concatenate(lower_condition_for_test, axis=2)
    lower_condition_for_test = lower_condition_for_test.transpose(2,0,1)[np.newaxis,...]

    upper_lower_norm_patches = np.concatenate([upper_lower_norm_patches, lower_condition_for_test], axis=1)
...
def setup_snapshot_image_grid(training_set, device, random_seed=0):
    ...
    images, poses, norm_img, norm_img_lower, _, _, _, Ms, M_invs, gt_parsings, _, _, \
        ...
        lower_clothes_upper_bounds_for_test, lower_conditions_for_train = zip(*[training_set[i] for i in grid_indices])
    ...
    lower_conditions_for_train = np.array(lower_conditions_for_train)
    ...
    grid_denorm_upper_ims, grid_denorm_lower_ims, grid_denorm_upper_masks, grid_denorm_lower_masks, \
        grid_norm_parts = zip(*[denorm_clothes(norm_patches, norm_patches_lower, \
        norm_clothes_mask, norm_clothes_mask_lower, lower_label_map, gt_parsings, lower_conditions_for_train, \
        lower_clothes_upper_bounds_for_test, Ms, M_invs, i % gnum, i // gnum, gnum) for i in range(gnum*gnum)])
...
def training_loop(
    ...
    common_kwargs_D_parsing = dict(c_dim=512, img_resolution=training_set.resolution, img_channels=(7+6))
    ...
    if rank == 0:
        ...
        c = torch.empty([batch_gpu, (10*3+5*3+1+3), 128, 128], device=device)
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            # 读取模型输入数据和GT
            phase_real, phase_pose, phase_norm_img, _,phase_norm_img_lower, phase_denorm_upper_img, phase_denorm_lower_img, _,_, \
                phase_gt_parsing, phase_denorm_upper_mask, phase_denorm_lower_mask, _, _, phase_retain_mask, \
                phase_skin, phase_lower_label_map, _, phase_lower_condition_for_train = next(training_set_iterator)

            phase_real_tensor = phase_real.to(device).to(torch.float32) / 127.5 - 1

            phase_lower_condition_for_train_tensor = phase_lower_condition_for_train.to(device).to(torch.float32) / 127.5 - 1
            ...
            phase_parts_tensor = torch.cat([phase_parts_tensor,phase_parts_lower_tensor,phase_lower_label_map_tensor,phase_lower_condition_for_train_tensor],dim=1)
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            ...
            for z, c, retain, denorm_upper_im, denorm_lower_im, denorm_upper_ma, denorm_lower_ma, pose in zip(grid_z, \
                ...
                parsing_index = parsing_index.cpu().numpy().astype(np.int32)
                parsing_index = np.concatenate([parsing_index,parsing_index,parsing_index], axis=1)
                # parsing_vis = np.zeros((1, 3, parsing_index.shape[2], parsing_index.shape[3]))
                # for h in range(parsing_index.shape[2]):
                #     for w in range(parsing_index.shape[3]):
                #         parsing_vis[0,:,h,w] = CMAP[parsing_index[0,0,h,w]]
            ...
            # pred_parsings = pred_parsings / 127.5 - 1.0
