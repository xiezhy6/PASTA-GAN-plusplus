...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, G_const_encoding, G_style_encoding, 
        ...
        class_weight  = torch.FloatTensor([1,2,2,2,3,3]).to(device)
    ...
    def run_G(self, z, c, pose, const_feats, denorm_upper_mask, denorm_lower_mask, \
            denorm_upper_input, denorm_lower_input, gt_parsing, sync):
        cat_feats = {}
        ...
        with misc.ddp_sync(self.G_synthesis, sync):
            finetune_img, pred_parsing = self.G_synthesis(ws, pose_feat, cat_feats, denorm_upper_input, \
                                                        denorm_lower_input, denorm_upper_mask, \
                                                        denorm_lower_mask, gt_parsing)

        return finetune_img, pred_parsing, ws
    ...
    def accumulate_gradients(self, phase, real_img, gen_z, style_input, retain, pose, denorm_upper_input, denorm_lower_input, denorm_upper_mask, denorm_lower_mask, \
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_finetune_img, pred_parsing, _gen_ws = self.run_G(gen_z, gen_c, pose, cat_feats, denorm_upper_mask, denorm_lower_mask, \
                                                                                                denorm_upper_input, denorm_lower_input, \
                                                                                                gt_parsing, sync=(sync and not do_Gpl)) # May get synced by Gpl.
                ...
                loss_Gmain_parsing = loss_Gmain_parsing.mean() * 0.5
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_finetune_img, _, _gen_ws = self.run_G(gen_z, gen_c, pose, cat_feats, denorm_upper_mask, denorm_lower_mask, \
                                                                        denorm_upper_input, denorm_lower_input, \
                                                                        gt_parsing, sync=False)
        ...
        if do_Dmain or do_Dr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                if do_Dmain:
                    ...
                    training_stats.report('Loss/D/loss', loss_Dgen_finetune + loss_Dreal)
        ...
        if do_DPmain:
            with torch.autograd.profiler.record_function('Dparsing_forward'):
                _, pred_parsing, _ = self.run_G(gen_z, gen_c, pose, cat_feats, denorm_upper_mask, denorm_lower_mask, \
                                                                        denorm_upper_input, denorm_lower_input, \
                                                                        gt_parsing, sync=False)
        ...
        if do_DPmain or do_DPr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                lower_pants_mask = (gt_parsing==2).to(gt_parsing.dtype).to(gt_parsing.device)
                lower_skirt_mask = (gt_parsing==3).to(gt_parsing.dtype).to(gt_parsing.device)
                neck_mask = (gt_parsing==4).to(gt_parsing.dtype).to(gt_parsing.device)
                hand_leg_mask = (gt_parsing==5).to(gt_parsing.dtype).to(gt_parsing.device)

                gt_parsing_tmp = torch.cat([bg_mask,upper_mask,lower_pants_mask,lower_skirt_mask,neck_mask,hand_leg_mask],dim=1).detach().requires_grad_(do_DPr1)
