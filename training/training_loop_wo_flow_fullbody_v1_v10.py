...
def combine_parts(parts, masks, col, row, gnum):
    ...
    col_norm_img_lower = col_part[30:45]
    col_norm_mask_upper = col_part[45:55]
    col_norm_mask_lower = col_part[55:]
    col_mask = masks[col]

    row_part = parts_tmp[row]
    row_norm_img_upper = row_part[:30]
    row_norm_img_lower = row_part[30:45]
    row_norm_mask_upper = row_part[45:55]
    row_norm_mask_lower = row_part[55:]
    row_mask = masks[row]
    ...
    if row < gap:
        # if row == 2:
        #     upper_patch_vis = (row_norm_img_upper[0:3,...].cpu().numpy().transpose(1,2,0) + 1) * 127.5
        #     upper_patch_vis = upper_patch_vis[...,[2,1,0]].astype(np.uint8)
        #     cv2.imwrite('z_debug_upper_patch.png', upper_patch_vis)
        #     patch_mask_vis = row_mask[0:1].cpu().numpy().transpose(1,2,0) * 255
        #     patch_mask_vis = np.concatenate([patch_mask_vis,patch_mask_vis,patch_mask_vis], axis=2)
        #     cv2.imwrite('z_debug_upper_patch_mask.png', patch_mask_vis)

        #     patch = col_norm_img_lower[0:3,...]
        #     patch_vis = (patch.cpu().numpy().transpose(1,2,0) + 1) * 127.5
        #     patch_vis = patch_vis[...,[2,1,0]].astype(np.uint8)
        #     cv2.imwrite('z_debug_patch_ori.png', patch_vis)

        # col_norm_img_lower[0:3,...] = col_norm_img_lower[0:3,...] * (1-row_mask[0:1]) - row_mask[0:1]
        # col_norm_img_lower[0:3,...] = col_norm_img_lower[0:3,...]
        col_norm_img_lower[0:3,...] = col_norm_img_lower[0:3,...] * (1-row_mask[0:1]) + (128/127.5-1.0) * row_mask[0:1]
        col_norm_mask_lower[0:1,...] = col_norm_mask_lower[0:1,...] * (1-row_mask[0:1]) - row_mask[0:1]
        
        # if row == 2:
        #     patch = col_norm_img_lower[0:3,...]
        #     patch_vis = (patch.cpu().numpy().transpose(1,2,0) + 1) * 127.5
        #     patch_vis = patch_vis[...,[2,1,0]].astype(np.uint8)
        #     cv2.imwrite('z_debug_patch.png', patch_vis)
        combined_part = torch.cat([row_norm_img_upper,col_norm_img_lower,row_norm_mask_upper,col_norm_mask_lower], dim=0)
    ###################### 换全身
    elif row < 2 * gap:
    ...
    ###################### 换上衣
        ...
        # row_norm_img_lower[0:3,...] = row_norm_img_lower[0:3,...]
        row_norm_img_lower[0:3,...] = row_norm_img_lower[0:3,...] * (1-col_mask[0:1]) + (128/127.5-1.0) * col_mask[0:1]
        row_norm_mask_lower[0:1,...] = row_norm_mask_lower[0:1,...] * (1-col_mask[0:1]) - col_mask[0:1]

        # combined_part = torch.cat([col_norm_img_upper,row_norm_img_lower], dim=0)
        combined_part = torch.cat([col_norm_img_upper,row_norm_img_lower,col_norm_mask_upper,row_norm_mask_lower], dim=0)
...
def denorm_clothes(norm_patches, norm_patches_lower, M_invs, norm_clothes_mask, norm_clothes_mask_lower, col, row, gnum):
    ...
    denorm_upper_img = np.ones((512,512,3),dtype=np.uint8) * 128
    denorm_lower_img = np.ones((512,512,3),dtype=np.uint8) * 128
    denorm_upper_mask = np.zeros((512,512,1),dtype=np.uint8)
    denorm_lower_mask = np.zeros((512,512,1),dtype=np.uint8)
    ...
    for ii in range(M_invs.shape[1]):
        ...
        if row < gap:
            ...
            norm_clothes_mask_patch = norm_clothes_mask[row,ii:(ii+1),...].transpose(1,2,0)
            norm_clothes_mask_patch = np.concatenate([norm_clothes_mask_patch,norm_clothes_mask_patch,norm_clothes_mask_patch],axis=2)
        ###### gap-(2*gap-1)换全身，gap-(gum-1)换上衣，上衣的索引是col
            ...
            norm_clothes_mask_patch = norm_clothes_mask[col,ii:(ii+1),...].transpose(1,2,0)
            norm_clothes_mask_patch = np.concatenate([norm_clothes_mask_patch,norm_clothes_mask_patch,norm_clothes_mask_patch],axis=2)
        ...
        if ii == 0:
            ...
            if row < 2 * gap:
                ...
                norm_clothes_mask_patch_lower = norm_clothes_mask_lower[col,ii:(ii+1),...].transpose(1,2,0)
                norm_clothes_mask_patch_lower = np.concatenate([norm_clothes_mask_patch_lower,norm_clothes_mask_patch_lower,norm_clothes_mask_patch_lower],axis=2)
            ########## gap-(gum-1)换上衣，裤子的索引是row
                ...
                norm_clothes_mask_patch_lower = norm_clothes_mask_lower[row,ii:(ii+1),...].transpose(1,2,0)
                norm_clothes_mask_patch_lower = np.concatenate([norm_clothes_mask_patch_lower,norm_clothes_mask_patch_lower,norm_clothes_mask_patch_lower],axis=2)
        ...
        if  ii >= 6:
            ...
            if row < 2 * gap:
                ...
                norm_clothes_mask_patch_lower = norm_clothes_mask_lower[col,(ii-6+1):(ii-6+2),...].transpose(1,2,0)
                norm_clothes_mask_patch_lower = np.concatenate([norm_clothes_mask_patch_lower,norm_clothes_mask_patch_lower,norm_clothes_mask_patch_lower],axis=2)
            ########## gap-(gum-1)换上衣，裤子的索引是row
            else:
                ...
                norm_clothes_mask_patch_lower = norm_clothes_mask_lower[row,(ii-6+1):(ii-6+2),...].transpose(1,2,0)
                norm_clothes_mask_patch_lower = np.concatenate([norm_clothes_mask_patch_lower,norm_clothes_mask_patch_lower,norm_clothes_mask_patch_lower],axis=2)
        ...
        denorm_upper_mask = denorm_clothes_mask_patch + denorm_upper_mask * (1-denorm_clothes_mask_patch)
        
        if ii == 0 or ii >= 6:
            # if ii == 0:
            #     norm_clothes_mask_patch_gray = (norm_clothes_mask_patch[...,0:1]>0).astype(np.uint8)
            #     norm_patch_lower = norm_patch_lower * (1-norm_clothes_mask_patch_gray)
            #     norm_clothes_mask_patch_lower = norm_clothes_mask_patch_lower * (1-norm_clothes_mask_patch_gray)
            ...
            # denorm_clothes_mask_patch_lower = denorm_clothes_mask_patch_lower[...,0:1]
            ...
            denorm_lower_mask = denorm_clothes_mask_patch_lower + denorm_lower_mask * (1-denorm_clothes_mask_patch_lower)

    denorm_upper_img = denorm_upper_img.transpose(2,0,1)[np.newaxis,...]
    denorm_lower_img = denorm_lower_img.transpose(2,0,1)[np.newaxis,...]
    denorm_upper_clothes_mask = denorm_upper_mask.transpose(2,0,1)[np.newaxis,...]
    denorm_lower_clothes_mask = denorm_lower_mask.transpose(2,0,1)[np.newaxis,...]
...
def setup_snapshot_image_grid(training_set, device, random_seed=0):
    ...
    # Load data.
    images, poses, norm_img, norm_img_lower, _, _, M_invs, _, _, _, norm_clothes_mask, norm_clothes_mask_lower, retain_masks, face_masks = zip(*[training_set[i] for i in grid_indices])
    ...
    norm_img = np.concatenate([norm_img, norm_img_lower, norm_clothes_mask, norm_clothes_mask_lower], axis=1)
    parts = np.array(norm_img)
    parts = torch.from_numpy(parts).to(device).to(torch.float32) / 127.5 - 1
    
    norm_mask = np.concatenate([norm_clothes_mask, norm_clothes_mask_lower], axis=1)[:,0:1,...]
    norm_mask = (norm_mask>0).astype(np.uint8)
    masks = torch.from_numpy(norm_mask).to(device).to(torch.float32)
    
    grid_norm_parts = [combine_parts(parts, masks, i % gnum, i // gnum, gnum) for i in range(gnum * gnum)]
    ...
    face_masks = torch.from_numpy(face_masks).to(device).to(torch.float32)

    grid_face_masks = [face_masks[i // gnum] for i in range(gnum * gnum)]
    grid_face_masks = torch.stack(grid_face_masks)
    ...
    return gnum, grid_ims, grid_poses, grid_norm_parts, grid_retains, grid_denorm_upper_ims, grid_denorm_lower_ims, \
           grid_denorm_upper_masks, grid_denorm_lower_masks, grid_face_masks
...
def training_loop(
    ...
    common_kwargs_D = dict(c_dim=512, img_resolution=training_set.resolution, img_channels=(training_set.num_channels+6))
    # common_kwargs_D_parsing = dict(c_dim=512, img_resolution=training_set.resolution, img_channels=(8+6))
    common_kwargs_D_parsing = dict(c_dim=512, img_resolution=training_set.resolution, img_channels=(5+6))
    ...
    if rank == 0:
        ...
        c = torch.empty([batch_gpu, (10*3+5*3+10+5), 128, 128], device=device)
        ...
        face_m = torch.empty([batch_gpu, 1, 512,512],device=device)
        img = misc.print_module_summary(G, [z, c, const_feature, pose, denorm_u, denorm_l, denorm_m_u, denorm_m_l, face_m])

        misc.print_module_summary(D, [torch.cat([img[0],pose],dim=1), c_feature])
        misc.print_module_summary(D_parsing, [torch.cat([img[2], pose],dim=1), c_feature])
    ...
    if rank == 0:
        ...
        gnum, grid_im, grid_pose, grid_part, grid_retain, \
            ...
            grid_denorm_lower_mask, grid_face_mask = setup_snapshot_image_grid(training_set=training_set, 
                                                               device=device,
        ...
        grid_face_masks = grid_face_mask.split(batch_gpu)
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            phase_real, phase_pose, phase_norm_img, phase_norm_img_lower, phase_denorm_upper_img, phase_denorm_lower_img, _, \
                phase_gt_parsing, phase_denorm_upper_mask, phase_denorm_lower_mask, phase_norm_upper_mask, phase_norm_lower_mask, \
                phase_retain_mask, phase_face_mask = next(training_set_iterator)
            ...
            phase_upper_mask_tensor = phase_norm_upper_mask.to(device).to(torch.float32) / 127.5 - 1
            phase_lower_mask_tensor = phase_norm_lower_mask.to(device).to(torch.float32) / 127.5 - 1

            phase_parts_tensor = torch.cat([phase_parts_tensor,phase_parts_lower_tensor,phase_upper_mask_tensor,phase_lower_mask_tensor],dim=1)
            ...
            phase_head_tensor = phase_retain_mask * phase_real_tensor - (1-phase_retain_mask)
            ...
            phase_face_mask_tensor = phase_face_mask.to(device).to(torch.float32)
            ...
            phase_face_mask_tensor = phase_face_mask_tensor.split(batch_gpu)
            ...
            del phase_retain_mask   # conserve memory
            ...
            del phase_norm_upper_mask
            del phase_norm_lower_mask
        ...
        for phase, phase_gen_z in zip(phases, all_gen_z):
            ...
            for round_idx, (real_img, gen_z, style_input, retain, pose, denorm_upper_input, denorm_lower_input, \
                            denorm_upper_mask, denorm_lower_mask, gt_parsing, face_mask) \
                    in enumerate(zip(phase_real_tensor, phase_gen_z, phase_parts_tensor, \
                                     ...
                                     phase_denorm_lower_mask_tensor, phase_gt_parsing_tensor, phase_face_mask_tensor)):
                ...
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, gen_z=gen_z, style_input=style_input, 
                                          ...
                                          denorm_lower_mask=denorm_lower_mask, gt_parsing=gt_parsing, face_mask=face_mask,
                                          sync=sync, gain=gain)
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            ...
            pred_parsings = []
            for z, c, retain, denorm_upper_im, denorm_lower_im, denorm_upper_ma, denorm_lower_ma, pose, face_mask in zip(grid_z, \
                    grid_parts, grid_retains, grid_denorm_upper_ims, grid_denorm_lower_ims, grid_denorm_upper_masks, grid_denorm_lower_masks, grid_poses, grid_face_masks):
                _, finetune_fake_image, pred_parsing = G_ema(z=z, c=c, retain=retain, pose=pose, denorm_upper_input=denorm_upper_im, denorm_lower_input=denorm_lower_im, \
                                                            denorm_upper_mask=denorm_upper_ma, denorm_lower_mask=denorm_lower_ma, face_mask=face_mask, noise_mode='const')
            ...
            pred_parsings = pred_parsings / 7 * 2 - 1.0

            # combine_parsing_image = 0.4 * finetune_fake_images + 0.6 * pred_parsings
            ...
            # save_image_grid(image_side, image_top, combine_parsing_image, os.path.join(run_dir, f'fakes{cur_nimg//1000:06d}_combine.png'), drange=[-1,1], grid_size=(gnum, gnum))
