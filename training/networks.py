...
@persistence.persistent_class
class SynthesisBlockV2(torch.nn.Module):
    def __init__(self,
        in_channels,                        # Number of input channels, 0 = first block.
        out_channels,                       # Number of output channels.
        w_dim,                              # Intermediate latent (W) dimensionality.
    ...
    def __init__(self,
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        for res in self.block_resolutions:
            ...
            use_fp16 = (res >= fp16_resolution)
            is_last = (res == self.img_resolution)
            block = SynthesisBlockV2(in_channels, out_channels, w_dim=w_dim, resolution=res,
                img_channels=img_channels, is_last=is_last, use_fp16=use_fp16, **block_kwargs)
            self.num_ws += block.num_conv
            if is_last:
                self.num_ws += block.num_torgb
            ...
            # if res == self.block_resolutions[-2]:
            #     self.mask_conv = nn.Sequential(*[Conv2dLayer(out_channels, 1, kernel_size=1),
            #                                     nn.Sigmoid()])
            #     self.merge_conv = nn.Sequential(*[Conv2dLayer(out_channels + 3, out_channels, kernel_size=1)])
...
class GeneratorV2(torch.nn.Module):
    def __init__(self,
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkV2(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
...
class GeneratorV3(torch.nn.Module):
    def __init__(self,
        ...
        self.const_encoding = ConstEncoderNetwork(input_nc=3 + 3, output_nc=512, ngf=64, n_downsampling=6) 
        self.style_encoding = StyleEncoderNetworkV2(input_nc=18 * 2, output_nc=512, ngf=64, n_downsampling=6) 
...
@persistence.persistent_class
class GeneratorV4(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.const_encoding = ConstEncoderNetwork(input_nc=3 + 3, output_nc=512, ngf=64, n_downsampling=6) 
        self.style_encoding = StyleEncoderNetworkV3(input_nc=3 + 3, output_nc=512, ngf=64, n_downsampling=6) 
...
##################################### for smpl flow online optimization #######################

@persistence.persistent_class
class SynthesisNetworkV3(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        for res in self.block_resolutions:
            ...
            if res >= 32 and res != 256:
                mask_conv = nn.Sequential(*[Conv2dLayer(out_channels, 1, kernel_size=1, activation='sigmoid')])
                merge_conv = nn.Sequential(*[Conv2dLayer(out_channels, out_channels, kernel_size=1)])
                rgb_conv = nn.Sequential(*[Conv2dLayer(3,out_channels,kernel_size=1)])
                # rgb_conv = []
                # rgb_in_channels = [3, 64, 128]
                # rgb_out_channels = [64, 128, out_channels]
                # for ii in range(3):
                #     rgb_conv += [Dense(rgb_in_channels,rgb_in_channels),
                #                 Conv2dLayer(rgb_in_channels,rgb_out_channels,kernel_size=3)]
                setattr(self, f'mask_conv{res}', mask_conv)
                setattr(self, f'merge_conv{res}', merge_conv)
                setattr(self, f'rgb_conv{res}', rgb_conv)
            # if res == self.block_resolutions[-2]:
            #     self.mask_conv = nn.Sequential(*[Conv2dLayer(out_channels, 1, kernel_size=1),
            #                                     nn.Sigmoid()])
            #     self.merge_conv = nn.Sequential(*[Conv2dLayer(out_channels + 3, out_channels, kernel_size=1)])
    ...
    # def forward(self, ws, pose_feat, cat_feat, rec_top, **block_kwargs):
    def forward(self, ws, pose_feat, cat_feat, warp_top, **block_kwargs):                                
        block_ws = []
        with torch.autograd.profiler.record_function('split_ws'):
            misc.assert_shape(ws, [None, self.num_ws, self.w_dim])
            ws = ws.to(torch.float32)
        ...
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            ...
            if res >= 32 and res != 256:
                n,c,h,w = x.shape
                mask_conv = getattr(self,f'mask_conv{res}')
                rgb_conv = getattr(self,f'rgb_conv{res}')
                merge_conv = getattr(self,f'merge_conv{res}')
                
                mask = mask_conv(x)
                warp_top = F.interpolate(warp_top,size=(h,w))
                warp_x = rgb_conv(warp_top)
                merge_x = warp_x * mask + x * (1-mask)
                merge_x = merge_conv(merge_x)
            # if res == 128:
            #     n, c ,h, w = x.shape
            #     mask = self.mask_conv(x)
            #     merge_top = mask * F.interpolate(rec_top, size=(h, w)) - (1 - mask)
...
@persistence.persistent_class
class GeneratorV5(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkV3(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
    ...
    # def forward(self, z, c, retain, pose, aff_pose, aff_top, lower, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs):
    def forward(self, z, c, retain, pose, warp_top, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs):
        ...
        # 这里可以用PSP的encoders提出各层feature替换ws, ws里面就用upsample再拼接
        # img = self.synthesis(ws, pose_feat, cat_feats, rec_top, **synthesis_kwargs)
        img = self.synthesis(ws, pose_feat, cat_feats, warp_top, **synthesis_kwargs)
        return img
###########################################################################################
...
################################ use residual block ######
@persistence.persistent_class
class StyleEncoderNetworkV4(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=4):
        super().__init__()        
        encoder = []
        encoder += [Conv2dLayer(input_nc, ngf, kernel_size=1)]
        ...
        for i in range(3):
            ...
            if i == 2:
                encoder += [Attention(ngf * mult_ins[i], use_sn=False)]
            encoder += [Dense(ngf * mult_in, ngf * mult_in),
                        ResBlock(ngf * mult_in, ngf * mult_out, kernel_size=3, down=2)]
            # encoder += [Dense(ngf * mult_in, ngf * mult_in),
            #             Conv2dLayer(ngf * mult_in, ngf * mult_out, kernel_size=3, down=2)]
        mult_ins = [8, 8, 16]
        mult_outs = [8, 16, 32]
        for i in range(3):
            ...
            encoder += [Dense(ngf * mult_in, ngf *mult_in)]
            if i < 1:
                encoder += [ResBlock(ngf * mult_in, ngf * mult_out, kernel_size=3)]
            else:
        ...
        encoder += [nn.AdaptiveAvgPool2d(1)]
        self.model = nn.Sequential(*encoder)
        self.fc = FullyConnectedLayer(2048, output_nc)
...
@persistence.persistent_class
class GeneratorV6(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.const_encoding = ConstEncoderNetwork(input_nc=3 + 3, output_nc=512, ngf=64, n_downsampling=6) 
        self.style_encoding = StyleEncoderNetworkV4(input_nc=24 * 2, output_nc=2048, ngf=64, n_downsampling=6) 
...
################## similar to swapping autoencoder without attention layer, dense layer ####################
@persistence.persistent_class
class StyleEncoderNetworkV5(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=4):
        super().__init__()        
        encoder = []
        encoder += [Conv2dLayer(input_nc, ngf, kernel_size=1)]
        ...
        for i in range(3):
            mult_in = mult_ins[i]
            mult_out = mult_outs[i]
            encoder += [ResBlock(ngf * mult_in, ngf * mult_out, kernel_size=3, down=2)]
        mult_ins = [8, 8, 16]
        mult_outs = [8, 16, 32]
        for i in range(3):
            mult_in = mult_ins[i]
...
@persistence.persistent_class
class GeneratorV7(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.const_encoding = ConstEncoderNetwork(input_nc=3 + 3, output_nc=512, ngf=64, n_downsampling=6) 
        self.style_encoding = StyleEncoderNetworkV5(input_nc=24 * 2, output_nc=2048, ngf=64, n_downsampling=6) 
...
######################### for new norm patch inputs ############################
@persistence.persistent_class
class GeneratorV8(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.const_encoding = ConstEncoderNetwork(input_nc=3 + 3, output_nc=512, ngf=64, n_downsampling=6) 
        self.style_encoding = StyleEncoderNetwork(input_nc=15 * 2 + 12, output_nc=512, ngf=64, n_downsampling=6) 
...
################################ concate patch feature in the decoder #############
@persistence.persistent_class
class StyleEncoderNetworkV6(nn.Module):
    def __init__(self, input_nc, output_nc, patch_input_nc, ngf=64, n_downsampling=4):
        super().__init__()        
        encoder = []
        encoder += [Conv2dLayer(input_nc, ngf, kernel_size=1)]
        mult_ins = [1, 2, 4]
        ...
        patch_enc_32 = ResBlock(patch_input_nc,64,kernel_size=3,up=1)
        # patch_enc_64 = ResBlock(patch_input_nc,64,kernel_size=3,up=2)
        # patch_enc_128 = ResBlock(patch_input_nc,64,kernel_size=3,up=4)
        setattr(self, f'patch_32', patch_enc_32)
        # setattr(self, f'patch_64', patch_enc_64)
        # setattr(self, f'patch_128', patch_enc_128)

    def forward(self, x, const_input, patch_input):
        const_feats = []
        for _, module in enumerate(self.feat_enc):
            const_input = module(const_input)
            const_feats.append(const_input)

        # patch_feats = [getattr(self, f'patch_{res}')(patch_input) for res in [32,64,128]]
        patch_feats = []
        # for res in [32,64,128]:
        for res in [32]:
            patch_enc = getattr(self, f'patch_{res}')
            patch_feat = patch_enc(patch_input)
        ...
        return x, const_feats, patch_feats

@persistence.persistent_class
class SynthesisBlockV4(torch.nn.Module):
    def __init__(self,
        in_channels,                        # Number of input channels, 0 = first block.
        out_channels,                       # Number of output channels.
        w_dim,                              # Intermediate latent (W) dimensionality.
        ...
        **layer_kwargs,                     # Arguments for SynthesisLayer.
        ...
        self.patch_merge_conv = Conv2dLayer(in_channels + 64, in_channels, kernel_size=1,
                                      resample_filter=resample_filter, channels_last=self.channels_last)

    def forward(self, x, img, ws, pose_feature, cat_feat, cat_patch_feat, force_fp32=False, fused_modconv=None, **layer_kwargs):
        misc.assert_shape(ws, [None, self.num_conv + self.num_torgb, self.w_dim])
        w_iter = iter(ws.unbind(dim=1))
        dtype = torch.float16 if self.use_fp16 and not force_fp32 else torch.float32
        memory_format = torch.channels_last if self.channels_last and not force_fp32 else torch.contiguous_format
        ...
        if self.in_channels == 0:
            ...
            # if x.shape[2] > 16 and x.shape[2] < 256:
            if x.shape[2] == 32:
                x = torch.cat([x, cat_patch_feat[str(x.shape[2])].to(dtype=dtype, memory_format=memory_format)],dim=1)
                x = self.patch_merge_conv(x)
...
@persistence.persistent_class
class SynthesisNetworkV4(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        for res in self.block_resolutions:
            ...
            use_fp16 = (res >= fp16_resolution)
            is_last = (res == self.img_resolution)
            block = SynthesisBlockV4(in_channels, out_channels, w_dim=w_dim, resolution=res,
                img_channels=img_channels, is_last=is_last, use_fp16=use_fp16, **block_kwargs)
            self.num_ws += block.num_conv
            if is_last:
                self.num_ws += block.num_torgb
    ...
    # def forward(self, ws, pose_feat, cat_feat, rec_top, **block_kwargs):
    def forward(self, ws, pose_feat, cat_feat, cat_patch_feat, **block_kwargs):
        block_ws = []
        with torch.autograd.profiler.record_function('split_ws'):
            misc.assert_shape(ws, [None, self.num_ws, self.w_dim])
            ws = ws.to(torch.float32)
        ...
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            block = getattr(self, f'b{res}')
            # x, img = block(x, img, cur_ws, pose_feat, cat_feat, rec_top, **block_kwargs)
            x, img = block(x, img, cur_ws, pose_feat, cat_feat, cat_patch_feat, **block_kwargs)

        return img


@persistence.persistent_class
class GeneratorV9(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkV4(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)

        self.const_encoding = ConstEncoderNetwork(input_nc=3 + 3, output_nc=512, ngf=64, n_downsampling=6) 
        self.style_encoding = StyleEncoderNetworkV6(input_nc=15 * 2, output_nc=512, patch_input_nc=36, ngf=64, n_downsampling=6) 
        
        # self.flownet = FlowNet(3 + 3 + 3 + 3)

    # def forward(self, z, c, retain, pose, aff_pose, aff_top, lower, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs):
    def forward(self, z, c, retain, pose, patch_inputs, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs):

        pose_feat = self.const_encoding(pose)
        stylecode, feats, patch_feats = self.style_encoding(c, retain, patch_inputs)
        ws = self.mapping(z, stylecode, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff)
        ...
        # 这里可以用PSP的encoders提出各层feature替换ws, ws里面就用upsample再拼接
        # img = self.synthesis(ws, pose_feat, cat_feats, rec_top, **synthesis_kwargs)
        img = self.synthesis(ws, pose_feat, cat_feats, cat_patch_feats, **synthesis_kwargs)
        return img
...
@persistence.persistent_class
class Spade_Norm_Block_v1(torch.nn.Module):
    def __init__(self,
        in_channels,
        norm_channels,
    ):
        super().__init__()
        self.conv_mlp = Spade_Conv2dLayer(in_channels, norm_channels, kernel_size=1, bias=False)
        self.conv_mlp_act = nn.ReLU()
        self.conv_gamma = Spade_Conv2dLayer(norm_channels, norm_channels, kernel_size=1, bias=False)
        self.conv_beta = Spade_Conv2dLayer(norm_channels, norm_channels, kernel_size=1, bias=False)
...
@persistence.persistent_class
class Spade_Norm_Block_v2(torch.nn.Module):
    def __init__(self,
        in_channels,
        norm_channels,
    ):
        super().__init__()
        self.conv_mlp = Spade_Conv2dLayer_partialconv(in_channels, norm_channels, kernel_size=3, bias=False)
        self.conv_mlp_act = nn.ReLU()
        self.conv_gamma = Spade_Conv2dLayer_partialconv(norm_channels, norm_channels, kernel_size=3, bias=False)
        self.conv_beta = Spade_Conv2dLayer_partialconv(norm_channels, norm_channels, kernel_size=3, bias=False)

        self.param_free_norm = nn.InstanceNorm2d(norm_channels, affine=False)

    def forward(self, x, denorm_feats, mask):
        normalized = self.param_free_norm(x)
        actv = self.conv_mlp(denorm_feats, mask, no_act=True)
        actv = self.conv_mlp_act(actv)
        gamma = self.conv_gamma(actv, mask, no_act=True)
        beta = self.conv_beta(actv, mask, no_act=True)
...
@persistence.persistent_class
class StyleEncoderNetworkV17(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=4):
        super().__init__()
        encoder = []
        encoder += [Conv2dLayer(input_nc, ngf, kernel_size=1)]
...
@persistence.persistent_class
class Spade_ResBlockV2_512(torch.nn.Module):
    def __init__(self,
        in_channels,                    # Number of input channels.
        out_channels,                   # Number of output channels.
        kernel_size     = 3,            # Width and height of the convolution kernel.
...
@persistence.persistent_class
class Spade_ResBlockV3_512(torch.nn.Module):
    def __init__(self,
        in_channels,                    # Number of input channels.
        out_channels,                   # Number of output channels.
        kernel_size     = 3,            # Width and height of the convolution kernel.
...
@persistence.persistent_class
class Spade_ResBlockV5_512(torch.nn.Module):
    def __init__(self,
        in_channels,                    # Number of input channels.
        out_channels,                   # Number of output channels.
        spade_channels, 
        ...
        self.conv = Spade_Conv2dLayer(in_channels, in_channels, kernel_size=1, bias=False,
                                resample_filter=resample_filter, conv_clamp=conv_clamp, channels_last=channels_last)
        self.conv0 = Spade_Conv2dLayer(in_channels, out_channels, kernel_size=1, bias=False, 
                                resample_filter=resample_filter, conv_clamp=conv_clamp, channels_last=channels_last)
        self.conv1 = Spade_Conv2dLayer(out_channels, out_channels, kernel_size=1, bias=False, 
                                resample_filter=resample_filter, conv_clamp=conv_clamp, channels_last=channels_last)
        self.skip = Spade_Conv2dLayer(in_channels, out_channels, kernel_size=1, bias=False, 
                                resample_filter=resample_filter, conv_clamp=conv_clamp, channels_last=channels_last)
        ...
        #     feat_channels = 64*2
        feat_channels = spade_channels
        self.spade_skip = Spade_Norm_Block_v1(feat_channels, in_channels)
        self.spade0 = Spade_Norm_Block_v1(feat_channels, in_channels)
        self.spade1 = Spade_Norm_Block_v1(feat_channels, out_channels)
...
@persistence.persistent_class
class Spade_ResBlockV6_512(torch.nn.Module):
    def __init__(self,
        in_channels,                    # Number of input channels.
        out_channels,                   # Number of output channels.
        spade_channels, 
        ...
        self.conv = Spade_Conv2dLayer_partialconv(in_channels, in_channels, kernel_size=3, bias=False,
                                resample_filter=resample_filter, conv_clamp=conv_clamp, channels_last=channels_last)
        self.conv0 = Spade_Conv2dLayer_partialconv(in_channels, out_channels, kernel_size=3, bias=False, 
                                resample_filter=resample_filter, conv_clamp=conv_clamp, channels_last=channels_last)
        self.conv1 = Spade_Conv2dLayer_partialconv(out_channels, out_channels, kernel_size=3, bias=False, 
                                resample_filter=resample_filter, conv_clamp=conv_clamp, channels_last=channels_last)
        self.skip = Spade_Conv2dLayer(in_channels, out_channels, kernel_size=1, bias=False, 
                                resample_filter=resample_filter, conv_clamp=conv_clamp, channels_last=channels_last)
        ...
        #     feat_channels = 64*2
        feat_channels = spade_channels
        self.spade_skip = Spade_Norm_Block_v2(feat_channels, in_channels)
        self.spade0 = Spade_Norm_Block_v2(feat_channels, in_channels)
        self.spade1 = Spade_Norm_Block_v2(feat_channels, out_channels)

    def forward(self, x, denorm_feat, mask):
        x = self.conv(x, mask, no_act=True)

        y = self.skip(self.spade_skip(x,denorm_feat,mask), gain=np.sqrt(0.5))
        x = self.conv0(self.spade0(x,denorm_feat,mask), mask)
        x = self.conv1(self.spade1(x,denorm_feat,mask), mask, gain=np.sqrt(0.5))
...
class SynthesisNetworkFull(torch.nn.Module):
    def __init__(self,
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        in_channels = channels_dict[res]
        out_channels = channels_dict[res]
        self.spade_b256_1 = Spade_ResBlockV2_512(in_channels, out_channels)
        self.spade_b256_2 = Spade_ResBlockV2_512(in_channels, out_channels)
        self.spade_b256_3 = Spade_ResBlockV2_512(in_channels, out_channels)

        res = self.block_resolutions[-1]
        in_channels = channels_dict[res//2]
        out_channels = channels_dict[res]
        self.texture_b512 = SynthesisBlockFull(in_channels, out_channels, w_dim=w_dim, resolution=res,
    ...
    def forward(self, ws, pose_feat, cat_feat, denorm_upper_input, denorm_lower_input, denorm_upper_mask, \
        ...
        cur_ws = block_ws[-1]
        finetune_x, finetune_img, _ = self.texture_b512(x_spade_256, img_256, cur_ws, pose_feat, cat_feat, force_fp32=True, **block_kwargs)
...
class GeneratorFull(torch.nn.Module):
    def __init__(self,
        ...
        self.const_encoding = ConstEncoderNetwork(input_nc=3 + 3, output_nc=512, ngf=64, n_downsampling=6) 
        self.style_encoding = StyleEncoderNetworkV16(input_nc=(10*3+5*3), output_nc=512, ngf=64, n_downsampling=6)
...
###########################  512 full body predict parsing 8 class  ###############################

@persistence.persistent_class
class ToRGBLayerFull_v1(torch.nn.Module):
    def __init__(self, in_channels, out_channels, w_dim, kernel_size=1, conv_clamp=None, channels_last=False, is_last=False, is_style=False):
        super().__init__()
        self.conv_clamp = conv_clamp
        self.affine = FullyConnectedLayer(w_dim, in_channels, bias_init=1)
        ...
        if self.is_last and self.is_style:
            self.m_weight1 = torch.nn.Parameter(torch.randn([8, in_channels, kernel_size, kernel_size]).to(memory_format=memory_format))
            self.m_bias1 = torch.nn.Parameter(torch.zeros([8]))
...
@persistence.persistent_class
class SynthesisBlockFull_v1(torch.nn.Module):
    def __init__(self,
        in_channels,                        # Number of input channels, 0 = first block.
        out_channels,                       # Number of output channels.
        w_dim,                              # Intermediate latent (W) dimensionality.
        ...
        **layer_kwargs,                     # Arguments for SynthesisLayer.
        ...
        if is_last or architecture == 'skip':
            self.torgb = ToRGBLayerFull_v1(out_channels, img_channels, w_dim=w_dim,
                conv_clamp=conv_clamp, channels_last=self.channels_last, 
                is_last=is_last, is_style=is_style)
            self.num_torgb += 1
...
@persistence.persistent_class
class SynthesisBlockFull_v1_v2(torch.nn.Module):
    def __init__(self,
        in_channels,                        # Number of input channels, 0 = first block.
        out_channels,                       # Number of output channels.
        w_dim,                              # Intermediate latent (W) dimensionality.
...
@persistence.persistent_class
class SynthesisBlockFull_v1_v3(torch.nn.Module):
    def __init__(self,
        in_channels,                        # Number of input channels, 0 = first block.
        out_channels,                       # Number of output channels.
        w_dim,                              # Intermediate latent (W) dimensionality.
        ...
        **layer_kwargs,                     # Arguments for SynthesisLayer.
        ...
        self.spade_b512 = Spade_ResBlockV5_512(out_channels, out_channels, spade_channels=1)
...
@persistence.persistent_class
class SynthesisBlockFull_v1_v5(torch.nn.Module):
    def __init__(self,
        in_channels,                        # Number of input channels, 0 = first block.
        out_channels,                       # Number of output channels.
        w_dim,                              # Intermediate latent (W) dimensionality.
...
@persistence.persistent_class
class SynthesisNetworkFull_v1(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        for res in self.block_resolutions:
            ...
            use_fp16 = False
            is_last = (res == self.img_resolution)
            block = SynthesisBlockFull_v1(in_channels, out_channels, w_dim=w_dim, resolution=res,
                img_channels=img_channels, is_last=is_last, is_style=True, use_fp16=use_fp16, **block_kwargs)
            self.num_ws += block.num_conv
            if is_last:
                self.num_ws += block.num_torgb
        ...
        in_channels = channels_dict[res//2]
        out_channels = channels_dict[res]
        self.texture_b512 = SynthesisBlockFull_v1(in_channels, out_channels, w_dim=w_dim, resolution=res,
                img_channels=img_channels, is_last=True, is_style=False, use_fp16=False, **block_kwargs)
...
###########################  512 full body predict parsing 5 class  ###############################

@persistence.persistent_class
class ToRGBLayerFull_v2(torch.nn.Module):
    def __init__(self, in_channels, out_channels, w_dim, kernel_size=1, conv_clamp=None, channels_last=False, is_last=False, is_style=False):
        super().__init__()
        self.conv_clamp = conv_clamp
        self.affine = FullyConnectedLayer(w_dim, in_channels, bias_init=1)
...
@persistence.persistent_class
class SynthesisBlockFull_v2(torch.nn.Module):
    def __init__(self,
        in_channels,                        # Number of input channels, 0 = first block.
        out_channels,                       # Number of output channels.
        w_dim,                              # Intermediate latent (W) dimensionality.
        ...
        **layer_kwargs,                     # Arguments for SynthesisLayer.
        ...
        if is_last or architecture == 'skip':
            self.torgb = ToRGBLayerFull_v2(out_channels, img_channels, w_dim=w_dim,
                conv_clamp=conv_clamp, channels_last=self.channels_last, 
                is_last=is_last, is_style=is_style)
            self.num_torgb += 1
...
@persistence.persistent_class
class SynthesisBlockFull_v2_v3(torch.nn.Module):
    def __init__(self,
        in_channels,                        # Number of input channels, 0 = first block.
        out_channels,                       # Number of output channels.
        w_dim,                              # Intermediate latent (W) dimensionality.
...
@persistence.persistent_class
class GeneratorFull_v1(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v1(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
...
@persistence.persistent_class
class SynthesisNetworkFull_v2(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        in_channels = channels_dict[res]
        out_channels = channels_dict[res]
        self.spade_b256_1 = Spade_ResBlockV3_512(in_channels, out_channels)
        self.spade_b256_2 = Spade_ResBlockV3_512(in_channels, out_channels)
        self.spade_b256_3 = Spade_ResBlockV3_512(in_channels, out_channels)
...
@persistence.persistent_class
class GeneratorFull_v2(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v2(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
...
@persistence.persistent_class
class SynthesisNetworkFull_v4(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        self.spade_b256_2 = Spade_ResBlockV4_512(in_channels, out_channels, spade_channels=1)

        res = self.block_resolutions[-1]
        in_channels = channels_dict[res//2]
        out_channels = channels_dict[res]
        self.texture_b512 = SynthesisBlockFull_v1_v2(in_channels, out_channels, w_dim=w_dim, resolution=res,
                img_channels=img_channels, is_last=True, is_style=False, use_fp16=False, **block_kwargs)
    ...
    def forward(self, ws, pose_feat, cat_feat, denorm_upper_input, denorm_lower_input, denorm_upper_mask, \
        ...
        parsing_index_256 = torch.nn.functional.interpolate(parsing_index,scale_factor=0.5,mode='nearest').detach()
        spade_feat = spade_upper_feat * upper_mask_256 + spade_lower_feat * lower_mask_256
...
@persistence.persistent_class
class GeneratorFull_v4(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v4(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
...
@persistence.persistent_class
class SynthesisNetworkFull_v4_debug(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
    ...
    def forward(self, ws, pose_feat, cat_feat, denorm_upper_input, denorm_lower_input, denorm_upper_mask, \
        ...
        #################### debug #####################
        denorm_upper_mask = (denorm_upper_mask>0.9).to(upper_mask.device).to(upper_mask.dtype)

        valid_mask = ((upper_mask+denorm_upper_mask)==2.0).to(upper_mask.device).to(upper_mask.dtype)
        res_mask = (upper_mask-valid_mask).to(upper_mask.device).to(upper_mask.dtype)

        imgs_vis = finetune_img * 0.5 + upper_mask * 0.5
        imgs_vis = imgs_vis.detach().cpu().numpy().transpose(0,2,3,1)
        imgs_vis = ((imgs_vis + 1) * 127.5).astype(np.uint8)

        # imgs_vis_res = finetune_img * 0.5 + res_mask * 0.5
        imgs_vis_res = finetune_img * res_mask
        imgs_vis_res = imgs_vis_res.detach().cpu().numpy().transpose(0,2,3,1)
        imgs_vis_res = ((imgs_vis_res + 1) * 127.5).astype(np.uint8)

        denorm_imgs_vis = denorm_upper_input * 0.5 + upper_mask * 0.5
        denorm_imgs_vis = denorm_imgs_vis.detach().cpu().numpy().transpose(0,2,3,1)
        denorm_imgs_vis = ((denorm_imgs_vis + 1) * 127.5).astype(np.uint8)

        for ii in range(imgs_vis.shape[0]):
            img_vis = imgs_vis[ii][...,[2,1,0]]
            img_vis_res = imgs_vis_res[ii][...,[2,1,0]]
            denorm_vis = denorm_imgs_vis[ii][...,[2,1,0]]
            import cv2
            cv2.imwrite('z_debug_result.png', img_vis)
            cv2.imwrite('z_debug_result_res.png', img_vis_res)
            cv2.imwrite('z_debug_denorm.png', denorm_vis)

        return img, finetune_img, pred_parsing


@persistence.persistent_class
class GeneratorFull_v4_debug(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v4_debug(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
...
@persistence.persistent_class
class SynthesisNetworkFull_v5(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        in_channels = channels_dict[res]
        out_channels = channels_dict[res]
        self.spade_b256_1 = Spade_ResBlockV5_512(in_channels, out_channels, spade_channels=128)
        self.spade_b256_2 = Spade_ResBlockV5_512(in_channels, out_channels, spade_channels=1)

        res = self.block_resolutions[-1]
        in_channels = channels_dict[res//2]
        out_channels = channels_dict[res]
        self.texture_b512 = SynthesisBlockFull_v1_v3(in_channels, out_channels, w_dim=w_dim, resolution=res,
                img_channels=img_channels, is_last=True, is_style=False, use_fp16=False, **block_kwargs)
...
####### use 1*1 conv in spade #######
@persistence.persistent_class
class GeneratorFull_v5(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v5(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
...
#### use partial conv for spade feature
@persistence.persistent_class
class Conv2dLayer_partialconv(torch.nn.Module):
    def __init__(self,
        in_channels,                    # Number of input channels.
        out_channels,                   # Number of output channels.
        kernel_size,                    # Width and height of the convolution kernel.
    ...
    def forward(self, x, mask, gain=1):
        w = self.weight * self.weight_gain
        b = self.bias.to(x.dtype) if self.bias is not None else None
        flip_weight = (self.up == 1) # slightly faster
        x = conv2d_resample.conv2d_resample(x=x, w=w.to(x.dtype), f=self.resample_filter, up=self.up, down=self.down, padding=self.padding, flip_weight=flip_weight)
...
@persistence.persistent_class
class SynthesisNetworkFull_v7(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        in_channels = channels_dict[res]
        out_channels = channels_dict[res]
        self.spade_b256_1 = Spade_ResBlockV6_512(in_channels, out_channels, spade_channels=128)
        self.spade_b256_2 = Spade_ResBlockV5_512(in_channels, out_channels, spade_channels=1)
    ...
    def forward(self, ws, pose_feat, cat_feat, denorm_upper_input, denorm_lower_input, denorm_upper_mask, \
        ...
        clothes_mask = ((upper_mask_256+lower_mask_256)>0).to(upper_mask.dtype).to(upper_mask.device)

        x_spade_256 = self.spade_b256_1(x_256, spade_feat, clothes_mask)
        x_spade_256 = self.spade_b256_2(x_spade_256, parsing_index_256)
        # x_spade_256 = self.spade_b256_3(x_spade_256, spade_feat)
...
@persistence.persistent_class
class GeneratorFull_v7(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v7(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
...
@persistence.persistent_class
class SynthesisNetworkFull_v8(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        spade_encoder = []
        ngf = 64
        spade_encoder += [Conv2dLayer_partialconv(3, ngf, kernel_size=7, activation='relu')]
        spade_encoder += [ResBlock_partialconv(ngf, ngf, kernel_size=3, activation='relu')]                 # 512
        spade_encoder += [ResBlock_partialconv(ngf, ngf*2, kernel_size=3, activation='relu', down=2)]       # 256
        # self.spade_encoder = nn.Sequential(*spade_encoder)
        self.spade_encoder = nn.ModuleList(spade_encoder)
    ...
    def get_spade_feat(self, mask_512, denorm_mask, denorm_input):
        ...
        denorm_input = denorm_input * mask_512 - (1-mask_512)
        valid_mask_512 = ((mask_512+denorm_mask)==2.0).to(mask_512.device).to(mask_512.dtype)
        # spade_denorm_feat = self.spade_encoder(denorm_input)
        spade_denorm_feat = denorm_input
        for ii, block in enumerate(self.spade_encoder):
            spade_denorm_feat = block(spade_denorm_feat, valid_mask_512)
...
@persistence.persistent_class
class GeneratorFull_v8(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v8(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
...
@persistence.persistent_class
class SynthesisNetworkFull_v9(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
    ...
    def get_skin_feat(self, retain, face_mask):
        face_mask_256 = torch.nn.functional.interpolate(face_mask,scale_factor=0.5)
        face_mask_256 = (face_mask_256>0.9).to(face_mask.device).to(face_mask.dtype)

        face = retain * face_mask - (1-face_mask)
        face_feat = self.spade_encoder(face)

        face_valid_feat = face_feat * face_mask_256

        valid_feat_sum = torch.sum(face_valid_feat, dim=(2,3), keepdim=True)
        valid_mask_sum = torch.sum(face_mask_256, dim=(2,3), keepdim=True)

        valid_index = (valid_mask_sum>10).to(face_mask.device).to(face_mask.dtype)
        valid_mask_sum = valid_mask_sum * valid_index + (256*256) * (1-valid_index)
        face_average_feat = valid_feat_sum / valid_mask_sum

        return face_average_feat


    def forward(self, ws, pose_feat, cat_feat, denorm_upper_input, denorm_lower_input, denorm_upper_mask, \
                denorm_lower_mask, retain, face_mask, **block_kwargs):
        block_ws = []
        with torch.autograd.profiler.record_function('split_ws'):
            misc.assert_shape(ws, [None, self.num_ws, self.w_dim])
            ws = ws.to(torch.float32)
        ...
        upper_mask = (parsing_index==1).float()
        lower_mask = (parsing_index==2).float()
        skin_mask = (parsing_index==3).float() + (parsing_index==4).float() + \
                    (parsing_index==5).float() + (parsing_index==6).float() + \
                    (parsing_index==7).float()

        spade_upper_feat = self.get_spade_feat(upper_mask.detach(), denorm_upper_mask, denorm_upper_input)
        spade_lower_feat = self.get_spade_feat(lower_mask.detach(), denorm_lower_mask, denorm_lower_input)
        spade_skin_feat = self.get_skin_feat(retain, face_mask)

        # spade_feat = torch.cat([spade_upper_feat, spade_lower_feat],dim=1)
        upper_mask_256 = torch.nn.functional.interpolate(upper_mask,scale_factor=0.5)
        lower_mask_256 = torch.nn.functional.interpolate(lower_mask,scale_factor=0.5)
        skin_mask_256 = torch.nn.functional.interpolate(skin_mask,scale_factor=0.5)
        upper_mask_256 = (upper_mask_256>0.9).to(upper_mask.dtype).to(upper_mask.device)
        lower_mask_256 = (lower_mask_256>0.9).to(upper_mask.dtype).to(upper_mask.device)
        skin_mask_256 = (skin_mask_256>0.9).to(upper_mask.dtype).to(upper_mask.device)

        parsing_index_256 = torch.nn.functional.interpolate(parsing_index,scale_factor=0.5,mode='nearest').detach()
        spade_feat = spade_upper_feat * upper_mask_256 + spade_lower_feat * lower_mask_256 + spade_skin_feat * skin_mask_256
...
@persistence.persistent_class
class SynthesisNetworkFull_v11(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        for res in self.block_resolutions:
            ...
            use_fp16 = False
            is_last = (res == self.img_resolution)
            block = SynthesisBlockFull_v2(in_channels, out_channels, w_dim=w_dim, resolution=res,
                img_channels=img_channels, is_last=is_last, is_style=True, use_fp16=use_fp16, **block_kwargs)
            self.num_ws += block.num_conv
            if is_last:
                self.num_ws += block.num_torgb
        ...
        in_channels = channels_dict[res//2]
        out_channels = channels_dict[res]
        self.texture_b512 = SynthesisBlockFull_v2_v3(in_channels, out_channels, w_dim=w_dim, resolution=res,
                img_channels=img_channels, is_last=True, is_style=False, use_fp16=False, **block_kwargs)
    ...
    def get_spade_feat(self, mask_512, denorm_mask, denorm_input):
        ...
        # ###### debug ########
        # denorm_input_np = denorm_input.detach().cpu().numpy()[0,...].transpose(1,2,0)
        # mask_512_np = mask_512.detach().cpu().numpy()[0,...].transpose(1,2,0)
        # denorm_input_np = denorm_input_np * 0.5 + mask_512_np * 0.5
        # denorm_input_np = (denorm_input_np + 1.) / 2. * 255.
        # denorm_input_np = np.clip(denorm_input_np, 0, 255).astype(np.uint8)
        # denorm_input_np = denorm_input_np[...,[2,1,0]]
        # cv2.imwrite('z_debug_denorm_ori.png', denorm_input_np)
        # #####################

        # denorm_input = denorm_input * mask_512 - (1-mask_512)
        denorm_input = denorm_input * mask_512 + (128/127.5-1.0) * (1-mask_512)
        ...
        # denorm_input_np = np.clip(denorm_input_np, 0, 255).astype(np.uint8)
        # denorm_input_np = denorm_input_np[...,[2,1,0]]
        # cv2.imwrite('z_debug_denorm_align.png', denorm_input_np)
        # #####################
    ...
    def get_skin_feat(self, retain, face_mask):
        ...
        # face = retain * face_mask - (1-face_mask)
        face = retain * face_mask + (128/127.5-1.0) * (1-face_mask)
        face_feat = self.spade_encoder(face)
        face_valid_feat = face_feat * face_mask_256
    ...
    def forward(self, ws, pose_feat, cat_feat, denorm_upper_input, denorm_lower_input, denorm_upper_mask, \
        ...
        upper_mask = (parsing_index==1).float().to(parsing_index.device)
        lower_mask = (parsing_index==2).float().to(parsing_index.device)
        skin_mask = (parsing_index==3).float().to(parsing_index.device) + (parsing_index==4).float().to(parsing_index.device)
        ...
        # spade_feat = torch.cat([spade_upper_feat, spade_lower_feat],dim=1)
        upper_mask_256 = torch.nn.functional.interpolate(upper_mask,scale_factor=0.5,mode='bilinear')
        lower_mask_256 = torch.nn.functional.interpolate(lower_mask,scale_factor=0.5,mode='bilinear')
        skin_mask_256 = torch.nn.functional.interpolate(skin_mask,scale_factor=0.5,mode='bilinear')
        # upper_mask_256 = (upper_mask_256>0.9).to(upper_mask.dtype).to(upper_mask.device)
        # lower_mask_256 = (lower_mask_256>0.9).to(upper_mask.dtype).to(upper_mask.device)
        # skin_mask_256 = (skin_mask_256>0.9).to(upper_mask.dtype).to(upper_mask.device)
...
@persistence.persistent_class
class GeneratorFull_v9(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v9(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)

        self.const_encoding = ConstEncoderNetwork(input_nc=3 + 3, output_nc=512, ngf=64, n_downsampling=6) 
        # self.style_encoding = StyleEncoderNetworkV17(input_nc=(10*3+5*3), output_nc=512, ngf=64, n_downsampling=6)
        self.style_encoding = StyleEncoderNetworkV16(input_nc=(10*3+5*3), output_nc=512, ngf=64, n_downsampling=6)

    # def forward(self, z, c, retain, pose, denorm_input, denorm_mask, denorm_clothes_mask=None, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs):
    def forward(self, z, c, retain, pose, denorm_upper_input, denorm_lower_input, denorm_upper_mask, denorm_lower_mask, \
         face_mask, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs):
        pose_feat = self.const_encoding(pose)
        stylecode, feats = self.style_encoding(c, retain)
        ws = self.mapping(z, stylecode, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff)
        ...
        img, finetune_img, pred_parsing = self.synthesis(ws, pose_feat, cat_feats, denorm_upper_input, denorm_lower_input, \
                                                denorm_upper_mask, denorm_lower_mask, retain, face_mask, **synthesis_kwargs)
        return img, finetune_img, pred_parsing

@persistence.persistent_class
class GeneratorFull_v11(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v11(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)

        self.const_encoding = ConstEncoderNetwork(input_nc=3 + 3, output_nc=512, ngf=64, n_downsampling=6) 
        self.style_encoding = StyleEncoderNetworkV16(input_nc=(10*3+5*3+10+5), output_nc=512, ngf=64, n_downsampling=6)
    ...
    def forward(self, z, c, retain, pose, denorm_upper_input, denorm_lower_input, denorm_upper_mask, denorm_lower_mask, \
        ...
        # lower_mask_np = lower_mask.detach().cpu().numpy()[0,...].transpose(1,2,0)

        # fake_np = finetune_img.detach().cpu().numpy()[0,...].transpose(1,2,0)
        # fake_np = fake_np * 0.5 + lower_mask_np * 0.5
        # fake_np = (fake_np +1.) / 2. * 255.
        # fake_np = np.clip(fake_np, 0, 255).astype(np.uint8)
        # fake_np = fake_np[...,[2,1,0]]
        # cv2.imwrite('z_debug_fake.png', fake_np)
        # ##############################
...
@persistence.persistent_class
class ToRGBLayerFull_v3_v1(torch.nn.Module):
    def __init__(self, in_channels, out_channels, w_dim, kernel_size=1, conv_clamp=None, channels_last=False, is_last=False, is_style=False):
        super().__init__()
        self.conv_clamp = conv_clamp
        self.affine = FullyConnectedLayer(w_dim, in_channels, bias_init=1)
...
@persistence.persistent_class
class ToRGBLayerFull_v3_v2(torch.nn.Module):
    def __init__(self, in_channels, out_channels, w_dim, kernel_size=1, conv_clamp=None, channels_last=False, is_last=False, is_style=False):
        super().__init__()
        self.conv_clamp = conv_clamp
        self.affine = FullyConnectedLayer(w_dim, in_channels, bias_init=1)
...
@persistence.persistent_class
class ToRGBLayerFull_v3_v2_v1(torch.nn.Module):
    def __init__(self, in_channels, out_channels, w_dim, kernel_size=1, conv_clamp=None, channels_last=False, is_last=False, is_style=False):
        super().__init__()
        self.conv_clamp = conv_clamp
        self.affine = FullyConnectedLayer(w_dim, in_channels, bias_init=1)
...
@persistence.persistent_class
class SynthesisBlockFull_v3_v1(torch.nn.Module):
    def __init__(self,
        in_channels,                        # Number of input channels, 0 = first block.
        out_channels,                       # Number of output channels.
        w_dim,                              # Intermediate latent (W) dimensionality.
        ...
        **layer_kwargs,                     # Arguments for SynthesisLayer.
        ...
        if is_last or architecture == 'skip':
            self.torgb = ToRGBLayerFull_v3_v1(out_channels, img_channels, w_dim=w_dim,
                conv_clamp=conv_clamp, channels_last=self.channels_last, 
                is_last=is_last, is_style=is_style)
            self.num_torgb += 1
...
@persistence.persistent_class
class SynthesisBlockFull_v3_v2(torch.nn.Module):
    def __init__(self,
        in_channels,                        # Number of input channels, 0 = first block.
        out_channels,                       # Number of output channels.
        w_dim,                              # Intermediate latent (W) dimensionality.
        ...
        **layer_kwargs,                     # Arguments for SynthesisLayer.
        ...
        if is_last or architecture == 'skip':
            self.torgb = ToRGBLayerFull_v3_v2(out_channels, img_channels, w_dim=w_dim,
                conv_clamp=conv_clamp, channels_last=self.channels_last, 
                is_last=is_last, is_style=is_style)
            self.num_torgb += 1
    ...
    def forward(self, x, img, ws, pose_feature, cat_feat, force_fp32=False, fused_modconv=None, **layer_kwargs):
        ...
        if img is not None:
            ...
            img = upfirdn2d.upsample2d(img, self.resample_filter)
        if self.is_last or self.architecture == 'skip':
            pred_parsing = self.torgb(x, next(w_iter), fused_modconv=fused_modconv)
            # y, pred_parsing = self.torgb(x, next(w_iter), fused_modconv=fused_modconv)
            # y = y.to(dtype=torch.float32, memory_format=torch.contiguous_format)
            # img = img.add_(y) if img is not None else y
...
@persistence.persistent_class
class SynthesisBlockFull_v3_v2_v1(torch.nn.Module):
    def __init__(self,
        in_channels,                        # Number of input channels, 0 = first block.
        out_channels,                       # Number of output channels.
        w_dim,                              # Intermediate latent (W) dimensionality.
        ...
        **layer_kwargs,                     # Arguments for SynthesisLayer.
        ...
        if is_last or architecture == 'skip':
            self.torgb = ToRGBLayerFull_v3_v2_v1(out_channels, img_channels, w_dim=w_dim,
                conv_clamp=conv_clamp, channels_last=self.channels_last, 
                is_last=is_last, is_style=is_style)
            self.num_torgb += 1
...
@persistence.persistent_class
class SynthesisBlockFull_v3_v3(torch.nn.Module):
    def __init__(self,
        in_channels,                        # Number of input channels, 0 = first block.
        out_channels,                       # Number of output channels.
        w_dim,                              # Intermediate latent (W) dimensionality.
...
@persistence.persistent_class
class SynthesisBlockFull_v3_v4(torch.nn.Module):
    def __init__(self,
        in_channels,                        # Number of input channels, 0 = first block.
        out_channels,                       # Number of output channels.
        w_dim,                              # Intermediate latent (W) dimensionality.
...
@persistence.persistent_class
class SynthesisNetworkFull_v13(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        for res in self.block_resolutions:
            ...
            is_last = (res == self.img_resolution)
            if not is_last:
                block = SynthesisBlockFull_v3_v1(in_channels, out_channels, w_dim=w_dim, resolution=res,
                    img_channels=img_channels, is_last=is_last, is_style=True, use_fp16=use_fp16, **block_kwargs)
            else:
                block = SynthesisBlockFull_v3_v2(in_channels, out_channels, w_dim=w_dim, resolution=res,
                    img_channels=img_channels, is_last=is_last, is_style=True, use_fp16=use_fp16, **block_kwargs)
            self.num_ws += block.num_conv
            if is_last:
                self.num_ws += block.num_torgb
        ...
        in_channels = channels_dict[res//2]
        out_channels = channels_dict[res]
        self.texture_b512 = SynthesisBlockFull_v3_v3(in_channels, out_channels, w_dim=w_dim, resolution=res,
                img_channels=img_channels, is_last=True, is_style=False, use_fp16=False, **block_kwargs)
    ...
    def forward(self, ws, pose_feat, cat_feat, denorm_upper_input, denorm_lower_input, denorm_upper_mask, \
        ...
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            block = getattr(self, f'b{res}')
            # x, img, pred_parsing = block(x, img, cur_ws, pose_feat, cat_feat, force_fp32=True, **block_kwargs)
            x, pred = block(x, img, cur_ws, pose_feat, cat_feat, force_fp32=True, **block_kwargs)
            if res == 256:
                x_256, img_256 = x.clone(), pred.clone()
        ...
        cur_ws = block_ws[-1]
        _, finetune_img = self.texture_b512(x_spade_256, img_256, cur_ws, pose_feat, cat_feat, parsing_index, \
                                                        force_fp32=True, **block_kwargs)

        return finetune_img, pred_parsing


@persistence.persistent_class
class GeneratorFull_v13(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v13(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
    ...
    def forward(self, z, c, retain, pose, denorm_upper_input, denorm_lower_input, denorm_upper_mask, denorm_lower_mask, \
        ...
        finetune_img, pred_parsing = self.synthesis(ws, pose_feat, cat_feats, denorm_upper_input, denorm_lower_input, \
                                                denorm_upper_mask, denorm_lower_mask, **synthesis_kwargs)
        return finetune_img, pred_parsing


@persistence.persistent_class
class SynthesisNetworkFull_v14(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
...
@persistence.persistent_class
class GeneratorFull_v14(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v14(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
...
@persistence.persistent_class
class SynthesisNetworkFull_v15(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        self.spade_b256_3 = Spade_ResBlockV4_512(in_channels, out_channels, spade_channels=128)
        # self.spade_b256_1 = Spade_ResBlockV5_512(in_channels, out_channels, spade_channels=128)
        # self.spade_b256_2 = Spade_ResBlockV5_512(in_channels, out_channels, spade_channels=1)
        ...
        in_channels = channels_dict[res//2]
        out_channels = channels_dict[res]
        self.texture_b512 = SynthesisBlockFull_v3_v4(in_channels, out_channels, w_dim=w_dim, resolution=res,
                img_channels=img_channels, is_last=True, is_style=False, use_fp16=False, **block_kwargs)
...
@persistence.persistent_class
class GeneratorFull_v15(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v15(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
...
@persistence.persistent_class
class SynthesisNetworkFull_v16(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        for res in self.block_resolutions:
            ...
            if not is_last:
                ...
                block = SynthesisBlockFull_v3_v2_v1(in_channels, out_channels, w_dim=w_dim, resolution=res,
                    img_channels=img_channels, is_last=is_last, is_style=True, use_fp16=use_fp16, **block_kwargs)
            self.num_ws += block.num_conv
            if is_last:
                self.num_ws += block.num_torgb
...
@persistence.persistent_class
class GeneratorFull_v16(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v16(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)

        self.const_encoding = ConstEncoderNetwork(input_nc=3 + 3, output_nc=512, ngf=64, n_downsampling=6) 
        self.style_encoding = StyleEncoderNetworkV18(input_nc=(10*3+5*3+1), output_nc=512, ngf=64, n_downsampling=6)
...
@persistence.persistent_class
class SynthesisNetworkFull_v17(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        for res in self.block_resolutions:
            ...
            use_fp16 = False
            is_last = (res == self.img_resolution)
            block = SynthesisBlockFull_v1_v5(in_channels, out_channels, w_dim=w_dim, resolution=res,
                img_channels=img_channels, is_last=is_last, is_style=True, use_fp16=use_fp16, **block_kwargs)
            self.num_ws += block.num_conv
            if is_last:
                self.num_ws += block.num_torgb
...
@persistence.persistent_class
class GeneratorFull_v17(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v17(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
...
@persistence.persistent_class
class GeneratorFull_v18(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.const_encoding = ConstEncoderNetwork(input_nc=3 + 3, output_nc=512, ngf=64, n_downsampling=6) 
        self.style_encoding = StyleEncoderNetworkV18(input_nc=(10*3+5*3+1+3), output_nc=512, ngf=64, n_downsampling=6)
...
@persistence.persistent_class
class SynthesisNetworkFull_v18_for_test(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
    ...
    def forward(self, ws, pose_feat, cat_feat, denorm_upper_input, denorm_lower_input, denorm_upper_mask, \
                denorm_lower_mask, gt_parsing, upper_preserves=None, lower_preserves=None, upper_preserve_masks=None, \
                lower_preserve_masks=None, preserve_lower_masks_ori=None, **block_kwargs):
        block_ws = []
        with torch.autograd.profiler.record_function('split_ws'):
            misc.assert_shape(ws, [None, self.num_ws, self.w_dim])
            ws = ws.to(torch.float32)
        ...
        ### 只用于测试半身试穿，使用部分gt paring替换pred parsing
        ### 上半身试穿，替换下半身parsing
        if lower_preserves is not None:
            gt_pants_masks = (lower_preserve_masks==2).float()
            gt_skirt_masks = (lower_preserve_masks==3).float()
            gt_leg_masks = (lower_preserve_masks==7).float()
            gt_palm_masks = (lower_preserve_masks==8).float()            

            lower_preserve_masks = (preserve_lower_masks_ori==2).float() + \
                                   (preserve_lower_masks_ori==3).float()

            pred_tops_masks = (parsing_index==1).float()
            pred_pants_masks = (parsing_index==2).float()
            pred_skirt_masks = (parsing_index==3).float()
            pred_dresses_masks = (parsing_index==4).float()
            pred_neck_masks = (parsing_index==5).float()
            pred_hand_leg_masks = (parsing_index==6).float()
            
            bs = gt_pants_masks.size(0)
            refine_parsings_list = []
            for ii in range(bs):
                gt_pants_mask_np = gt_pants_masks[ii].cpu().numpy().transpose(1,2,0)
                gt_skirt_mask_np = gt_skirt_masks[ii].cpu().numpy().transpose(1,2,0)
                gt_leg_mask_np = gt_leg_masks[ii].cpu().numpy().transpose(1,2,0)
                gt_palm_mask_np = gt_palm_masks[ii].cpu().numpy().transpose(1,2,0)

                pred_tops_mask_np = pred_tops_masks[ii].cpu().numpy().transpose(1,2,0)
                pred_pants_mask_np = pred_pants_masks[ii].cpu().numpy().transpose(1,2,0)
                pred_skirt_mask_np = pred_skirt_masks[ii].cpu().numpy().transpose(1,2,0)
                pred_dresses_mask_np = pred_dresses_masks[ii].cpu().numpy().transpose(1,2,0)
                pred_neck_mask_np = pred_neck_masks[ii].cpu().numpy().transpose(1,2,0)
                pred_hand_leg_mask_np = pred_hand_leg_masks[ii].cpu().numpy().transpose(1,2,0)

                pred_lower_mask_np = pred_pants_mask_np + pred_skirt_mask_np

                lower_bbox = self.mask_to_bbox(pred_lower_mask_np)
                if lower_bbox is not None:
                    if np.sum(gt_leg_mask_np) > 0:
                        lower_clothes_lower_bound = lower_bbox[3]
                        lower_clothes_lower_bound -= 10
                        pred_hand_leg_mask_np[lower_clothes_lower_bound:] *= 0

                    lower_clothes_upper_bound = lower_bbox[1]
                    gt_pants_mask_np[:lower_clothes_upper_bound] *= 0
                    gt_skirt_mask_np[:lower_clothes_upper_bound] *= 0
                
                pred_hand_leg_mask_np = ((pred_hand_leg_mask_np+gt_palm_mask_np)>0).astype(np.float32)
                pred_upper_mask_np = pred_tops_mask_np + pred_neck_mask_np + pred_hand_leg_mask_np
                
                gt_pants_mask_np *= (1-pred_upper_mask_np)
                gt_skirt_mask_np *= (1-pred_upper_mask_np)

                pred_hand_leg_mask_np = ((pred_hand_leg_mask_np+gt_leg_mask_np)>0).astype(np.float32)
                refine_parsing = pred_tops_mask_np * 1 + gt_pants_mask_np * 2 + \
                                 gt_skirt_mask_np * 3 + pred_dresses_mask_np * 4 + \
                                 pred_neck_mask_np * 5 + pred_hand_leg_mask_np * 6
                
                refine_parsing = refine_parsing.transpose(2,0,1)[np.newaxis,...]
                refine_parsings_list.append(refine_parsing)

            refine_parsings = np.concatenate(refine_parsings_list,axis=0)
            parsing_index = torch.tensor(refine_parsings).to(parsing_index.dtype).to(parsing_index.device)

            denorm_lower_input = lower_preserves
            denorm_lower_mask = lower_preserve_masks
        ...
        return img, finetune_img, pred_parsing, parsing_index


@persistence.persistent_class
class GeneratorFull_v20_for_test(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_v18_for_test(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
    ...
    def forward(self, z, c, retain, pose, denorm_upper_input, denorm_lower_input, denorm_upper_mask, denorm_lower_mask, \
                gt_parsing=None, upper_preserve=None, lower_preserve=None, upper_preserve_mask=None, lower_preserve_mask=None, \
                preserve_lower_masks_ori=None, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs):
        pose_feat = self.const_encoding(pose)
        stylecode, feats = self.style_encoding(c, retain)
        ws = self.mapping(z, stylecode, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff)
        ...
        img, finetune_img, pred_parsing, parsing_index = self.synthesis(ws, pose_feat, cat_feats, denorm_upper_input, denorm_lower_input, \
                                                                        denorm_upper_mask, denorm_lower_mask, gt_parsing, upper_preserve, lower_preserve, \
                                                                        upper_preserve_mask, lower_preserve_mask, preserve_lower_masks_ori, **synthesis_kwargs)
        return img, finetune_img, pred_parsing, parsing_index


###########################  512 full body predict parsing 9 class  end ###############################



################ 512 full body with gated conv, use downsampling and upsampling, with average feature inpainting ###################

@persistence.persistent_class
class GatedConv2dLayer(torch.nn.Module):
    def __init__(self,
        in_channels,                    # Number of input channels.
        out_channels,                   # Number of output channels.
        kernel_size,                    # Width and height of the convolution kernel.
        ...
        super().__init__()
        self.conv2d = Conv2dLayer(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel_size, 
                                 bias=bias, activation=activation, up=up, down=down)
        self.mask_conv2d = Conv2dLayer(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel_size, 
                                 bias=bias, activation='sigmoid', up=up, down=down)
...
@persistence.persistent_class
class SynthesisNetworkFull_gated_v2(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
        ...
        **block_kwargs,             # Arguments for SynthesisBlock.
        ...
        inpaint_network = []
        ngf = 64
        inpaint_network += [GatedConv2dLayer(3, ngf, kernel_size=7, activation='relu')]
        inpaint_network += [GatedConv2dLayer(ngf, ngf, kernel_size=3, activation='relu')]           # 512 * 512
        inpaint_network += [GatedConv2dLayer(ngf, ngf*2, kernel_size=3, activation='relu', down=2)]
        # inpaint_network += [GatedConv2dLayer(ngf*2, ngf*2, kernel_size=3, activation='relu')]         # 256 * 256
        inpaint_network += [GatedConv2dLayer(ngf*2, ngf*4, kernel_size=3, activation='relu', down=2)]
        # inpaint_network += [GatedConv2dLayer(ngf*4, ngf*4, kernel_size=3, activation='relu')]         # 128 * 128
        inpaint_network += [GatedConv2dLayer(ngf*4, ngf*2, kernel_size=3, activation='relu', up=2)]
        inpaint_network += [GatedConv2dLayer(ngf*2, ngf*2, kernel_size=3, activation='relu')]        # 256 * 256

        self.inpaint_network = nn.Sequential(*inpaint_network)
    ...
    def get_spade_feat(self, mask_512, denorm_mask, denorm_input):
        ...
        denorm_input = denorm_input * mask_512 - (1-mask_512)
        # spade_denorm_feat = self.spade_encoder(denorm_input)
        spade_denorm_feat = self.inpaint_network(denorm_input)
        spade_denorm_valid_feat = spade_denorm_feat * valid_mask
...
@persistence.persistent_class
class GeneratorFull_gated_v2(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_gated_v2(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
...
################ 512 full body with gated conv  only downsampling  end###################

@persistence.persistent_class
class SynthesisNetworkFull_gated_v3(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
        img_resolution,             # Output image resolution.
        img_channels,               # Number of color channels.
    ...
    def get_spade_feat(self, mask_512, denorm_mask, denorm_input):
        ...
        # spade_denorm_feat = self.spade_encoder(denorm_input)
        spade_denorm_feat = self.inpaint_network(denorm_input)
        spade_denorm_feat = spade_denorm_feat * mask_256
        spade_denorm_valid_feat = spade_denorm_feat * valid_mask
...
@persistence.persistent_class
class GeneratorFull_gated_v3(torch.nn.Module):
    def __init__(self,
        z_dim,                      # Input latent (Z) dimensionality.
        c_dim,                      # Conditioning label (C) dimensionality.
        w_dim,                      # Intermediate latent (W) dimensionality.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = SynthesisNetworkFull_gated_v3(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, **synthesis_kwargs)
        self.num_ws = self.synthesis.num_ws
        self.mapping = MappingNetwork(z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws, **mapping_kwargs)
