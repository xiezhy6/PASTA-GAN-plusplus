...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, G_const_encoding, G_style_encoding, 
        ...
        class_weight  = torch.FloatTensor([1,3,3,5,5,3,3]).to(device)
    ...
    def run_G(self, z, c, pose, const_feats, denorm_upper_mask, denorm_lower_mask, \
        ...
        with misc.ddp_sync(self.G_synthesis, sync):
            img, finetune_img, pred_parsing = self.G_synthesis(ws, pose_feat, cat_feats, denorm_upper_input, \
    ...
    def accumulate_gradients(self, phase, real_img, gen_z, style_input, retain, pose, denorm_upper_input, denorm_lower_input, denorm_upper_mask, denorm_lower_mask, \
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, gen_finetune_img, pred_parsing, _ = self.run_G(gen_z, gen_c, pose, cat_feats, denorm_upper_mask, denorm_lower_mask, \
        ...
        if do_DPmain or do_DPr1:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                dresses_mask = (gt_parsing==4).to(gt_parsing.dtype).to(gt_parsing.device)
                neck_mask = (gt_parsing==5).to(gt_parsing.dtype).to(gt_parsing.device)
                hand_leg_mask = (gt_parsing==6).to(gt_parsing.dtype).to(gt_parsing.device)

                gt_parsing_tmp = torch.cat([bg_mask,upper_mask,lower_pants_mask,lower_skirt_mask,dresses_mask,neck_mask,hand_leg_mask],dim=1).detach().requires_grad_(do_DPr1)
