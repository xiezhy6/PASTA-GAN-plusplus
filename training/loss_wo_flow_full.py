...
class StyleGAN2Loss(Loss):
    ...
    def run_G(self, z, c, pose, const_feats, denorm_upper_mask, denorm_lower_mask, denorm_upper_input, denorm_lower_input, sync):
        ...
        with misc.ddp_sync(self.G_synthesis, sync):
            img, finetune_img, upper_mask, lower_mask = self.G_synthesis(ws, pose_feat, cat_feats, denorm_upper_input, denorm_lower_input, denorm_upper_mask, denorm_lower_mask)

        return img, finetune_img, upper_mask, lower_mask, ws
    ...
    def accumulate_gradients(self, phase, real_img, gen_z, style_input, retain, pose, denorm_upper_input, denorm_lower_input, denorm_upper_mask, denorm_lower_mask, \
                             upper_clothes_mask, lower_clothes_mask, sync, gain):
        ...
        if do_Gmain:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                gen_img, gen_finetune_img, gen_upper_mask, gen_lower_mask, _gen_ws = self.run_G(gen_z, gen_c, pose, cat_feats, denorm_upper_mask, denorm_lower_mask, \
                ...
                if self.mask_weight > 0:
                    upper_loss_mask = torch.nn.L1Loss()(gen_upper_mask, upper_clothes_mask) * self.mask_weight
                    # upper_loss_mask = upper_loss_mask.mean()
                    lower_loss_mask = torch.nn.L1Loss()(gen_lower_mask, lower_clothes_mask) * self.mask_weight
                    loss_mask = (upper_loss_mask + lower_loss_mask) / 2.0

                training_stats.report('Loss/G/mask_loss', loss_mask)
                training_stats.report('Loss/G/upper_mask_loss',upper_loss_mask)
                training_stats.report('Loss/G/lower_mask_loss', lower_loss_mask)
        ...
        if do_Dmain:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                gen_img, gen_finetune_img, _, _, _gen_ws = self.run_G(gen_z, gen_c, pose, cat_feats, denorm_upper_mask, denorm_lower_mask, \
